 ...
 * Example: npx tsx scripts/export-memories.ts "windows" windows-memories.json --project=ultrabrain
 */
...
async function exportMemories(query: string, outputFile: string, project?: string) {
  try {
    ...
    const settings = SettingsDefaultsManager.loadFromFile(join(homedir(), '.ultrabrain', 'settings.json'));
...
if (args.length < 2) {
  ...
  console.error('Example: npx tsx scripts/export-memories.ts "windows" windows-memories.json --project=ultrabrain');
