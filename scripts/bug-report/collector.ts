...
async function getDatabaseInfo(
  ...
  try {
    const dbPath = path.join(dataDir, "ultrabrain.db");
...
export async function collectDiagnostics(
  ...
  const homeDir = os.homedir();
  const dataDir = path.join(homeDir, ".ultrabrain");
  ...
  const isDevMode = cwd.includes("ultrabrain") && !cwd.includes(".claude");
  ...
  const database = {
    path: sanitizePath(path.join(dataDir, "ultrabrain.db")),
...
export function formatDiagnostics(diagnostics: SystemDiagnostics): string {
  ...
  output += "## Environment\n\n";
  output += `- **UltraBrain**: ${diagnostics.versions.claudeMem}\n`;
