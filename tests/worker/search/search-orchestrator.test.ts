...
describe('SearchOrchestrator', () => {
  ...
  describe('with Lance available', () => {
    beforeEach(() => {
      orchestrator = new SearchOrchestrator(mockSessionSearch, mockSessionStore, mockLanceSync);
    });
    ...
    describe('search', () => {
      it('should select SQLite strategy for filter-only queries (no query text)', async () => {
        ...
        expect(mockLanceSync.queryLance).not.toHaveBeenCalled();
      });

      it('should select Lance strategy for query-only', async () => {
        const result = await orchestrator.search({
          query: 'semantic search query'
        });

        expect(result.strategy).toBe('lance');
        expect(result.usedLance).toBe(true);
        expect(mockLanceSync.queryLance).toHaveBeenCalled();
      });

      it('should fall back to SQLite when Lance fails', async () => {
        ...
        // Lance failed, should have fallen back
    ...
    describe('findByConcept', () => {
      it('should use hybrid strategy when Lance available', async () => {
    ...
    describe('findByFile', () => {
      ...
      it('should include usedLance in result', async () => {
        const result = await orchestrator.findByFile('/path/to/file.ts', {});

        expect(typeof result.usedLance).toBe('boolean');
      });
    });

    describe('isLanceAvailable', () => {
      it('should return true when Lance is available', () => {
        expect(orchestrator.isLanceAvailable()).toBe(true);
      });
    });
    ...
    describe('formatSearchResults', () => {
      ...
      it('should indicate Lance failure when lanceFailed is true', () => {
        const results = {
          observations: [],
          sessions: [],
          prompts: []
  ...
  describe('without Lance (null)', () => {
    beforeEach(() => {
      orchestrator = new SearchOrchestrator(mockSessionSearch, mockSessionStore, null);
    });

    describe('isLanceAvailable', () => {
      it('should return false when Lance is null', () => {
        expect(orchestrator.isLanceAvailable()).toBe(false);
      });
    });

    describe('search', () => {
      it('should return empty results for query search without Lance', async () => {
        const result = await orchestrator.search({
          query: 'semantic query'
        });

        // No Lance available, can't do semantic search
