...
describe('HybridSearchStrategy', () => {
  ...
  let mockLanceSync: any;
  let mockSessionStore: any;
  let mockSessionSearch: any;

  beforeEach(() => {
    mockLanceSync = {
      queryLance: mock(() => Promise.resolve({
        ids: [2, 1, 3], // Lance returns in semantic relevance order
    ...
    mockSessionStore = {
      getObservationsByIds: mock((ids: number[]) => {
        // Return in the order we stored them (not Lance order)
    ...
    strategy = new HybridSearchStrategy(mockLanceSync, mockSessionStore, mockSessionSearch);
  });
  ...
  describe('canHandle', () => {
    ...
    it('should return false for filter-only without Lance', () => {
      // Create strategy without Lance
      const strategyNoLance = new HybridSearchStrategy(null as any, mockSessionStore, mockSessionSearch);

      const options: StrategySearchOptions = {
        concepts: ['test-concept']
      };
      expect(strategyNoLance.canHandle(options)).toBe(false);
    });
  });
  ...
  describe('findByConcept', () => {
    it('should combine metadata + semantic results', async () => {
      ...
      expect(mockLanceSync.queryLance).toHaveBeenCalledWith('test-concept', expect.any(Number));
      expect(result.usedLance).toBe(true);
      expect(result.fellBack).toBe(false);
      expect(result.strategy).toBe('hybrid');
    });

    it('should preserve semantic ranking order from Lance', async () => {
      // Lance returns: [2, 1, 3] (obs 2 is most relevant)
      // SQLite returns: [1, 2, 3] (by date or however)
      // Result should be in Lance order: [2, 1, 3]
      ...
      // The first result should be id=2 (Lance's top result)
      expect(result.results.observations[0].id).toBe(2);
    });

    it('should only include observations that match both metadata and Lance', async () => {
      // Metadata returns ids [1, 2, 3]
      // Lance returns ids [2, 4, 5] (4 and 5 don't exist in metadata results)
      mockLanceSync.queryLance = mock(() => Promise.resolve({
        ids: [2, 4, 5],
        distances: [0.1, 0.2, 0.3],
        metadatas: []
      }));
    ...
    it('should return empty when no metadata matches', async () => {
      ...
      expect(result.results.observations).toHaveLength(0);
      expect(mockLanceSync.queryLance).not.toHaveBeenCalled(); // Should short-circuit
    });

    it('should fall back to metadata-only on Lance error', async () => {
      mockLanceSync.queryLance = mock(() => Promise.reject(new Error('Lance failed')));
      ...
      expect(result.usedLance).toBe(false);
  ...
  describe('findByType', () => {
    it('should find observations by type with semantic ranking', async () => {
      ...
      expect(mockLanceSync.queryLance).toHaveBeenCalled();
      expect(result.usedLance).toBe(true);
    });
    ...
    it('should handle array of types', async () => {
      ...
      // Lance query should use joined type string
      expect(mockLanceSync.queryLance).toHaveBeenCalledWith('decision, bugfix', expect.any(Number));
    });

    it('should preserve Lance ranking order for types', async () => {
      mockLanceSync.queryLance = mock(() => Promise.resolve({
        ids: [2, 1], // Lance order
        distances: [0.1, 0.2],
        metadatas: []
    ...
    it('should fall back on Lance error', async () => {
      mockLanceSync.queryLance = mock(() => Promise.reject(new Error('Lance unavailable')));
  ...
  describe('findByFile', () => {
    ...
    it('should apply semantic ranking only to observations', async () => {
      mockLanceSync.queryLance = mock(() => Promise.resolve({
        ids: [2, 1], // Lance ranking for observations
        distances: [0.1, 0.2],
        metadatas: []
      }));
      ...
      // Observations should be in Lance order
      expect(result.observations[0].id).toBe(2);
      expect(result.usedLance).toBe(true);
    });

    it('should return usedLance: false when no observations to rank', async () => {
    ...
    it('should fall back on Lance error', async () => {
      mockLanceSync.queryLance = mock(() => Promise.reject(new Error('Lance down')));
