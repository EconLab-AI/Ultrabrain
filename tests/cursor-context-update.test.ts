 ...
 * .cursor/rules/ultrabrain-context.mdc for registered projects.
 *
...
describe('Cursor Context Update', () => {
  ...
  describe('writeContextFile', () => {
    ...
    it('creates ultrabrain-context.mdc file', () => {
      writeContextFile(workspacePath, 'test context');
    ...
    it('includes description in frontmatter', () => {
      ...
      const content = readContextFile(workspacePath);
      expect(content).toContain('description: "UltraBrain context from past sessions (auto-updated)"');
    });
...
- Added new feature`;
    ...
    it('includes footer with MCP tools mention', () => {
      ...
      const content = readContextFile(workspacePath);
      expect(content).toContain("Use ultrabrain's MCP search tools for more detailed queries");
    });

    it('uses atomic write (no temp file left behind)', () => {
      writeContextFile(workspacePath, 'test context');

      const tempFile = join(workspacePath, '.cursor', 'rules', 'ultrabrain-context.mdc.tmp');
