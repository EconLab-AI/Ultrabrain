...
describe('GeminiAgent', () => {
  ...
  beforeEach(() => {
    ...
    loadFromFileSpy = spyOn(SettingsDefaultsManager, 'loadFromFile').mockImplementation(() => ({
      ...
      ULTRABRAIN_GEMINI_API_KEY: 'test-api-key',
      ULTRABRAIN_GEMINI_MODEL: 'gemini-2.5-flash-lite',
      ULTRABRAIN_GEMINI_RATE_LIMITING_ENABLED: rateLimitingEnabled,
      ULTRABRAIN_DATA_DIR: '/tmp/ultrabrain-test',
    }));

    getSpy = spyOn(SettingsDefaultsManager, 'get').mockImplementation((key: string) => {
      if (key === 'ULTRABRAIN_GEMINI_API_KEY') return 'test-api-key';
      if (key === 'ULTRABRAIN_GEMINI_MODEL') return 'gemini-2.5-flash-lite';
      if (key === 'ULTRABRAIN_GEMINI_RATE_LIMITING_ENABLED') return rateLimitingEnabled;
      if (key === 'ULTRABRAIN_DATA_DIR') return '/tmp/ultrabrain-test';
  ...
  it('should respect rate limits when rate limiting enabled', async () => {
    ...
    // Note: ULTRABRAIN_GEMINI_RATE_LIMITING_ENABLED !== 'false' means enabled
