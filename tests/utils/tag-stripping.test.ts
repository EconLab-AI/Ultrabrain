 ...
 *
 * Tests the dual-tag privacy system for <private> and <ultrabrain-context> tags.
...
describe('Tag Stripping Utilities', () => {
  ...
  describe('stripMemoryTagsFromPrompt', () => {
    describe('basic tag removal', () => {
      ...
      it('should strip single <ultrabrain-context> tag', () => {
        const input = 'public content <ultrabrain-context>injected context</ultrabrain-context> more public';
        const result = stripMemoryTagsFromPrompt(input);
        expect(result).toBe('public content  more public');
      });

      it('should strip both tag types in mixed content', () => {
        const input = '<private>secret</private> public <ultrabrain-context>context</ultrabrain-context> end';
        const result = stripMemoryTagsFromPrompt(input);
    ...
    describe('multiple tags handling', () => {
      ...
      it('should strip multiple <ultrabrain-context> blocks', () => {
        const input = '<ultrabrain-context>ctx1</ultrabrain-context><ultrabrain-context>ctx2</ultrabrain-context> content';
        const result = stripMemoryTagsFromPrompt(input);
      ...
      it('should handle many interleaved tags', () => {
        let input = 'start';
        for (let i = 0; i < 10; i++) {
          input += ` <private>p${i}</private> <ultrabrain-context>c${i}</ultrabrain-context>`;
        }
        input += ' end';
        const result = stripMemoryTagsFromPrompt(input);
        // Tags are stripped but spaces between them remain
        expect(result).not.toContain('<private>');
        expect(result).not.toContain('<ultrabrain-context>');
    ...
    describe('empty and private-only prompts', () => {
      ...
      it('should return empty string for entirely context-tagged prompt', () => {
        const input = '<ultrabrain-context>all is context</ultrabrain-context>';
        const result = stripMemoryTagsFromPrompt(input);
        expect(result).toBe('');
      });
      ...
      it('should handle whitespace-only after stripping', () => {
        const input = '<private>content</private>   <ultrabrain-context>more</ultrabrain-context>';
        const result = stripMemoryTagsFromPrompt(input);
        expect(result).toBe('');
      });
    });
...
secret
      ...
      it('should strip multiline content within <ultrabrain-context> tags', () => {
        const input = `start
<ultrabrain-context>
# Recent Activity
- Item 1
...
finish`;
  ...
  describe('stripMemoryTagsFromJson', () => {
    describe('JSON content stripping', () => {
      ...
      it('should strip ultrabrain-context tags from JSON', () => {
        const jsonContent = JSON.stringify({
          data: '<ultrabrain-context>injected</ultrabrain-context> real data'
        });
        const result = stripMemoryTagsFromJson(jsonContent);
        const parsed = JSON.parse(result);
      ...
      it('should handle tool_response with tags', () => {
        const toolResponse = {
          output: 'result <ultrabrain-context>context data</ultrabrain-context>',
          status: 'success'
        };
