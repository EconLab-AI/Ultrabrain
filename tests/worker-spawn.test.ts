...
const TEST_DATA_DIR = path.join(homedir(), '.ultrabrain-test');
...
describe('Windows-specific behavior', () => {
  ...
  afterEach(() => {
    ...
    delete process.env.ULTRABRAIN_MANAGED;
  });
  ...
  it('should detect Windows managed worker mode correctly', () => {
    ...
    process.env.ULTRABRAIN_MANAGED = 'true';

    const isWindows = process.platform === 'win32';
    const isManaged = process.env.ULTRABRAIN_MANAGED === 'true';
