...
describe('Cursor MCP Configuration', () => {
  ...
  describe('configureCursorMcp', () => {
    ...
    it('adds ultrabrain server with correct structure', () => {
      configureCursorMcp(mcpJsonPath, mcpServerPath);

      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));

      expect(config.mcpServers).toBeDefined();
      expect(config.mcpServers['ultrabrain']).toBeDefined();
      expect(config.mcpServers['ultrabrain'].command).toBe('node');
      expect(config.mcpServers['ultrabrain'].args).toEqual([mcpServerPath]);
    });

    it('preserves existing MCP servers when adding ultrabrain', () => {
    ...
    it('updates existing ultrabrain server path', () => {
      // First config
      ...
      expect(config.mcpServers['ultrabrain'].args).toEqual([newPath]);
    });
  ...
  describe('removeMcpConfig', () => {
    it('removes ultrabrain server from config', () => {
      configureCursorMcp(mcpJsonPath, mcpServerPath);
      removeMcpConfig(mcpJsonPath);

      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(config.mcpServers['ultrabrain']).toBeUndefined();
    });

    it('preserves other servers when removing ultrabrain', () => {
      // Setup: both servers
      mkdirSync(join(tempDir, '.cursor'), { recursive: true });
      const config = {
        mcpServers: {
          'other-server': { command: 'python', args: ['/path.py'] },
          'ultrabrain': { command: 'node', args: ['/mcp.cjs'] }
        }
      };
      ...
      expect(updated.mcpServers['ultrabrain']).toBeUndefined();
    });
    ...
    it('does nothing if ultrabrain not in config', () => {
      mkdirSync(join(tempDir, '.cursor'), { recursive: true });
      const config = {
        mcpServers: {
          'other-server': { command: 'python', args: ['/path.py'] }
  ...
  describe('path handling', () => {
    it('handles absolute path with spaces', () => {
      ...
      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(config.mcpServers['ultrabrain'].args).toEqual([pathWithSpaces]);
    });
    ...
    it('handles Windows-style path', () => {
      ...
      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(config.mcpServers['ultrabrain'].args).toEqual([windowsPath]);
    });
    ...
    it('handles path with special characters', () => {
      ...
      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(config.mcpServers['ultrabrain'].args).toEqual([specialPath]);

      // Verify it survives JSON round-trip
      const reread: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(reread.mcpServers['ultrabrain'].args![0]).toBe(specialPath);
    });
  });
});
