...
describe('SettingsDefaultsManager', () => {
  ...
  describe('loadFromFile', () => {
    describe('file does not exist', () => {
      ...
      it('should write pretty-printed JSON (2-space indent)', () => {
        ...
        expect(content).toContain('  "ULTRABRAIN_MODEL"');
      });
    ...
    describe('file exists with valid content', () => {
      it('should return parsed content when file has valid JSON', () => {
        const customSettings = {
          ULTRABRAIN_MODEL: 'custom-model',
          ULTRABRAIN_WORKER_PORT: '12345',
        };
        writeFileSync(settingsPath, JSON.stringify(customSettings));

        const result = SettingsDefaultsManager.loadFromFile(settingsPath);

        expect(result.ULTRABRAIN_MODEL).toBe('custom-model');
        expect(result.ULTRABRAIN_WORKER_PORT).toBe('12345');
      });

      it('should merge file settings with defaults for missing keys', () => {
        // Only set one value, defaults should fill the rest
        const partialSettings = {
          ULTRABRAIN_MODEL: 'partial-model',
        };
        ...
        expect(result.ULTRABRAIN_MODEL).toBe('partial-model');
        // Other values should come from defaults
        expect(result.ULTRABRAIN_WORKER_PORT).toBe(defaults.ULTRABRAIN_WORKER_PORT);
        expect(result.ULTRABRAIN_WORKER_HOST).toBe(defaults.ULTRABRAIN_WORKER_HOST);
        expect(result.ULTRABRAIN_LOG_LEVEL).toBe(defaults.ULTRABRAIN_LOG_LEVEL);
      });

      it('should not modify existing file when loading', () => {
        const customSettings = {
          ULTRABRAIN_MODEL: 'do-not-change',
      ...
      it('should handle all settings keys correctly', () => {
        ...
        fullSettings.ULTRABRAIN_MODEL = 'all-keys-model';
        fullSettings.ULTRABRAIN_PROVIDER = 'gemini';
        writeFileSync(settingsPath, JSON.stringify(fullSettings));

        const result = SettingsDefaultsManager.loadFromFile(settingsPath);

        expect(result.ULTRABRAIN_MODEL).toBe('all-keys-model');
        expect(result.ULTRABRAIN_PROVIDER).toBe('gemini');
      });
    });
    ...
    describe('nested schema migration', () => {
      it('should migrate old nested { env: {...} } schema to flat schema', () => {
        const nestedSettings = {
          env: {
            ULTRABRAIN_MODEL: 'nested-model',
            ULTRABRAIN_WORKER_PORT: '54321',
          },
        };
        ...
        expect(result.ULTRABRAIN_MODEL).toBe('nested-model');
        expect(result.ULTRABRAIN_WORKER_PORT).toBe('54321');
      });

      it('should auto-migrate file from nested to flat schema', () => {
        const nestedSettings = {
          env: {
            ULTRABRAIN_MODEL: 'migrated-model',
          },
        };
        writeFileSync(settingsPath, JSON.stringify(nestedSettings));
        ...
        expect(parsed.ULTRABRAIN_MODEL).toBe('migrated-model');
      });
    });
  ...
  describe('getAllDefaults', () => {
    ...
    it('should include all expected keys', () => {
      ...
      // Core settings
      expect(defaults.ULTRABRAIN_MODEL).toBeDefined();
      expect(defaults.ULTRABRAIN_WORKER_PORT).toBeDefined();
      expect(defaults.ULTRABRAIN_WORKER_HOST).toBeDefined();

      // Provider settings
      expect(defaults.ULTRABRAIN_PROVIDER).toBeDefined();
      expect(defaults.ULTRABRAIN_GEMINI_API_KEY).toBeDefined();
      expect(defaults.ULTRABRAIN_OPENROUTER_API_KEY).toBeDefined();

      // System settings
      expect(defaults.ULTRABRAIN_DATA_DIR).toBeDefined();
      expect(defaults.ULTRABRAIN_LOG_LEVEL).toBeDefined();
    });
  });
  ...
  describe('getInt', () => {
    it('should return integer value for numeric string', () => {
      expect(SettingsDefaultsManager.getInt('ULTRABRAIN_WORKER_PORT')).toBe(37777);
      expect(SettingsDefaultsManager.getInt('ULTRABRAIN_CONTEXT_OBSERVATIONS')).toBe(50);
    });
  });

  describe('getBool', () => {
    it('should return true for "true" string', () => {
      expect(SettingsDefaultsManager.getBool('ULTRABRAIN_CONTEXT_SHOW_READ_TOKENS')).toBe(true);
    });

    it('should return false for non-"true" string', () => {
      expect(SettingsDefaultsManager.getBool('ULTRABRAIN_CONTEXT_SHOW_LAST_MESSAGE')).toBe(false);
    });
  });
  ...
  describe('environment variable overrides', () => {
    ...
    beforeEach(() => {
      ...
      originalEnv.ULTRABRAIN_WORKER_PORT = process.env.ULTRABRAIN_WORKER_PORT;
      originalEnv.ULTRABRAIN_MODEL = process.env.ULTRABRAIN_MODEL;
      originalEnv.ULTRABRAIN_LOG_LEVEL = process.env.ULTRABRAIN_LOG_LEVEL;
    });

    afterEach(() => {
      // Restore original env values
      if (originalEnv.ULTRABRAIN_WORKER_PORT === undefined) {
        delete process.env.ULTRABRAIN_WORKER_PORT;
      } else {
        process.env.ULTRABRAIN_WORKER_PORT = originalEnv.ULTRABRAIN_WORKER_PORT;
      }
      if (originalEnv.ULTRABRAIN_MODEL === undefined) {
        delete process.env.ULTRABRAIN_MODEL;
      } else {
        process.env.ULTRABRAIN_MODEL = originalEnv.ULTRABRAIN_MODEL;
      }
      if (originalEnv.ULTRABRAIN_LOG_LEVEL === undefined) {
        delete process.env.ULTRABRAIN_LOG_LEVEL;
      } else {
        process.env.ULTRABRAIN_LOG_LEVEL = originalEnv.ULTRABRAIN_LOG_LEVEL;
      }
    });
    ...
    it('should prioritize env var over file setting', () => {
      ...
      process.env.ULTRABRAIN_WORKER_PORT = '54321';
    ...
    it('should prioritize env var over default', () => {
      // No file, env var set
      process.env.ULTRABRAIN_WORKER_PORT = '99999';

      const result = SettingsDefaultsManager.loadFromFile(settingsPath);

      expect(result.ULTRABRAIN_WORKER_PORT).toBe('99999');
    });

    it('should use file setting when env var is not set', () => {
      const fileSettings = {
        ULTRABRAIN_WORKER_PORT: '11111',
      };
      writeFileSync(settingsPath, JSON.stringify(fileSettings));
      delete process.env.ULTRABRAIN_WORKER_PORT;
      ...
      expect(result.ULTRABRAIN_WORKER_PORT).toBe('11111');
    });

    it('should apply env var override even on file parse error', () => {
      writeFileSync(settingsPath, 'invalid json {{{');
      process.env.ULTRABRAIN_WORKER_PORT = '88888';

      const result = SettingsDefaultsManager.loadFromFile(settingsPath);

      expect(result.ULTRABRAIN_WORKER_PORT).toBe('88888');
    });

    it('should apply multiple env var overrides', () => {
      const fileSettings = {
        ULTRABRAIN_WORKER_PORT: '12345',
        ULTRABRAIN_MODEL: 'file-model',
        ULTRABRAIN_LOG_LEVEL: 'DEBUG',
      };
      writeFileSync(settingsPath, JSON.stringify(fileSettings));

      process.env.ULTRABRAIN_WORKER_PORT = '54321';
      process.env.ULTRABRAIN_MODEL = 'env-model';
      // LOG_LEVEL not set in env, should use file value

      const result = SettingsDefaultsManager.loadFromFile(settingsPath);

      expect(result.ULTRABRAIN_WORKER_PORT).toBe('54321');
      expect(result.ULTRABRAIN_MODEL).toBe('env-model');
      expect(result.ULTRABRAIN_LOG_LEVEL).toBe('DEBUG'); // From file
    });
    ...
    it('should document priority: env > file > defaults', () => {
      ...
      const fileSettings = {
        ULTRABRAIN_WORKER_PORT: '22222', // Different from default 37777
      };
      writeFileSync(settingsPath, JSON.stringify(fileSettings));

      // Set env to something different from both
      process.env.ULTRABRAIN_WORKER_PORT = '33333';
      ...
      expect(defaults.ULTRABRAIN_WORKER_PORT).toBe('37777'); // Confirm default
      expect(result.ULTRABRAIN_WORKER_PORT).toBe('33333'); // Env wins
    });
  });
});
