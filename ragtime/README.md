...
Email Investigation Batch Processor using UltraBrain's email-investigation mode.

## Overview

Ragtime processes email corpus files through Claude, using the email-investigation mode for entity/relationship/timeline extraction. Each file gets a NEW session - context is managed by UltraBrain's context injection hook, not by conversation continuation.
...
| `RAGTIME_PLUGIN_PATH` | `./plugin` | Path to ultrabrain plugin |
| `ULTRABRAIN_WORKER_PORT` | `37777` | Worker service port |
...
1. **Startup**: Sets `ULTRABRAIN_MODE=email-investigation` and cleans up old transcripts
2. **Processing**: For each file:
   ...
   - UltraBrain's context injection hook provides relevant past observations
