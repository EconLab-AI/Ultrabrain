 ...
 * Each file gets a NEW session - context is managed by UltraBrain's context
...
const CONFIG = {
  ...
  // Path to ultrabrain plugin
  pluginPath: process.env.RAGTIME_PLUGIN_PATH ||
    path.join(process.cwd(), "plugin"),

  // Worker port
  workerPort: parseInt(process.env.ULTRABRAIN_WORKER_PORT || "37777", 10),
...
// Set email-investigation mode for UltraBrain
process.env.ULTRABRAIN_MODE = "email-investigation";
 ...
 * Context is injected by UltraBrain hooks, not conversation continuation
 */
