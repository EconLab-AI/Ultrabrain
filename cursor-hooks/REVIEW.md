...
3. **Parity with Claude Code**: Matches ultrabrain's hook behavior where possible
...
**Purpose**: Initialize ultrabrain session when prompt is submitted
...
- ✅ Observation format matches ultrabrain structure
...
**How It Works**:
1. Fetches context from ultrabrain worker
2. Writes to `.cursor/rules/ultrabrain-context.mdc` with `alwaysApply: true`
...
6. Write context to `.cursor/rules/ultrabrain-context.mdc`
...
3. **Context Injection**:
   - ✅ Solved via auto-updated `.cursor/rules/ultrabrain-context.mdc`
