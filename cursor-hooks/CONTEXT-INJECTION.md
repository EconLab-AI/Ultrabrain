...
1. **Install**: `ultrabrain cursor install` creates initial context file
2. **Stop hook**: `session-summary.sh` updates context after each session ends
3. **Cursor**: Automatically includes `.cursor/rules/ultrabrain-context.mdc` in all chats
...
3. Fetches existing context from ultrabrain and writes to `.cursor/rules/ultrabrain-context.mdc`
...
cat > .cursor/rules/ultrabrain-context.mdc << EOF
---
alwaysApply: true
---
# Memory Context
...
Located at: `.cursor/rules/ultrabrain-context.mdc`

```markdown
---
alwaysApply: true
description: "UltraBrain context from past sessions (auto-updated)"
---

# Memory Context from Past Sessions

[Your context from ultrabrain appears here]
...
When you run `ultrabrain cursor install`, the project is registered in `~/.ultrabrain/cursor-projects.json`. This allows the worker to automatically update your context file whenever a new summary is generated - even if it happens from Claude Code or another IDE working on the same project.

To see registered projects:
```bash
cat ~/.ultrabrain/cursor-projects.json
```
...
| Initial setup | Automatic | `ultrabrain cursor install` creates initial context |
| MCP tool access | ✅ Full support | ✅ Full support |
| Web viewer | ✅ Available | ✅ Available |

## First Session Behavior

When you run `ultrabrain cursor install`:
...
Configure ultrabrain's MCP server in Cursor for search tools:
...
Ask the agent: "Check ultrabrain for any previous work on authentication"

## File Location

The context file is created at:
```
<workspace>/.cursor/rules/ultrabrain-context.mdc
```

This is version-controlled by default. Add to `.gitignore` if you don't want to commit it:
```
.cursor/rules/ultrabrain-context.mdc
```
