# UltraBrain for Cursor (No Claude Code Required)

> **Persistent AI Memory for Cursor - Zero Cost to Start**

## Overview

Use ultrabrain's persistent memory in Cursor without a Claude Code subscription. Choose between free-tier providers (Gemini, OpenRouter) or paid options.
...
**Why This Matters**: Every Cursor session starts fresh. UltraBrain bridges that gap - your AI agent builds cumulative knowledge about your codebase, decisions, and patterns over time.
...
Since you don't have Claude Code, you need to configure an AI provider for ultrabrain's summarization engine.
...
cat > ~/.ultrabrain/settings.json << 'EOF'
{
  "ULTRABRAIN_PROVIDER": "gemini",
  "ULTRABRAIN_GEMINI_API_KEY": "YOUR_GEMINI_API_KEY",
...
cat > ~/.ultrabrain/settings.json << 'EOF'
{
  "ULTRABRAIN_PROVIDER": "openrouter",
  "ULTRABRAIN_OPENROUTER_API_KEY": "YOUR_OPENROUTER_API_KEY"
}
EOF
```
...
```bash
# From the ultrabrain repo directory (recommended - all projects)
...
- Visit https://docs.ultrabrain.ai for full documentation
...
$settingsDir = "$env:USERPROFILE\.ultrabrain"
