# UltraBrain Cursor Hooks Integration
...
cd ultrabrain && bun install && bun run build
...
The hooks bridge Cursor's hook system to ultrabrain's worker API, allowing:
...
1. **Install**: `ultrabrain cursor install` generates initial context
2. **Stop hook**: Updates context in `.cursor/rules/ultrabrain-context.mdc` after each session
...
- **MCP Tools**: Configure ultrabrain's MCP server for `search`, `timeline`, `get_observations` tools
...
|-------------|--------|---------|
| `beforeSubmitPrompt` | `session-init.sh` | Initialize ultrabrain session |
...
- Initializes a new session in ultrabrain using `conversation_id` as the session ID
- Extracts project name from workspace root
- Outputs `{"continue": true}` to allow prompt submission

### Context Hook (`context-inject.sh`)
- Ensures ultrabrain worker is running before session
- Outputs `{"continue": true}` to allow prompt submission
...
- Maps them to ultrabrain's observation format
...
- Requests summary generation from ultrabrain
- **Updates context file** in `.cursor/rules/ultrabrain-context.mdc` for next session

## Configuration

The hooks read configuration from `~/.ultrabrain/settings.json`:

- `ULTRABRAIN_WORKER_PORT`: Worker port (default: 37777)
- `ULTRABRAIN_WORKER_HOST`: Worker host (default: 127.0.0.1)
...
2. Check worker logs:
   ```bash
   tail -f ~/.ultrabrain/logs/worker-$(date +%Y-%m-%d).log
   ```
...
- [UltraBrain Documentation](https://docs.ultrabrain.ai)
- [Cursor Hooks Reference](../docs/context/cursor-hooks-reference.md)
- [UltraBrain Architecture](https://docs.ultrabrain.ai/architecture/overview)
