...
`+JSON.stringify(o,null,2):m=" "+this.formatData(o));let l="";if(n){let{sessionId:E,memorySessionId:g,correlationId:S,..._}=n;Object.keys(_).length>0&&(l=` {${Object.entries(_).map(([f,h])=>`${f}=${h}`).join(", ")}}`)}let T=`[${i}] [${a}] [${d}] ${p}${s}${l}${m}`;if(this.logFilePath)try{(0,L.appendFileSync)(this.logFilePath,T+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),m=p?`${p[1].split("/").pop()}:${p[2]}`:"unknown",l={...s,location:m};return this.warn(e,`[HAPPY-PATH] ${t}`,l,n),o}},u=new V;var yt={};function At(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,le.fileURLToPath)(yt.url))}var Lt=At(),I=C.get("ULTRABRAIN_DATA_DIR"),v=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,ue.homedir)(),".claude"),Jt=(0,b.join)(v,"plugins","marketplaces","EconLab-AI"),zt=(0,b.join)(I,"archives"),Qt=(0,b.join)(I,"logs"),Zt=(0,b.join)(I,"trash"),es=(0,b.join)(I,"backups"),ts=(0,b.join)(I,"modes"),ss=(0,b.join)(I,"settings.json"),_e=(0,b.join)(I,"ultrabrain.db"),rs=(0,b.join)(I,"vector-db"),ns=(0,b.join)(I,"observer-sessions"),os=(0,b.join)(v,"settings.json"),is=(0,b.join)(v,"commands"),as=(0,b.join)(v,"CLAUDE.md");function Ee(r){(0,me.mkdirSync)(r,{recursive:!0})}function Te(){return(0,b.join)(Lt,"..")}var P=class{db;constructor(e=_e){e!==":memory:"&&Ee(I),this.db=new ge.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addSessionSourceColumn(),this.createTasksTable(),this.createTagsSystem()}initializeSchema(){this.db.run(`
    ...
    `),this.db.run("CREATE INDEX IF NOT EXISTS idx_tasks_project ON tasks(project)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_tasks_status ON tasks(project, status)"),u.info("DB","Created tasks table for Kanban board"))}createTagsSystem(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(23))return;if(this.db.query("SELECT name FROM sqlite_master WHERE type='table' AND name='tags'").all().length>0){this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(23,new Date().toISOString());return}u.debug("DB","Creating tags system tables"),this.db.run(`
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,type:i,concepts:a,files:d}=t,p=s==="date_asc"?"ASC":"DESC",m=n?`LIMIT ${n}`:"",l=e.map(()=>"?").join(","),T=[...e],E=[];if(o&&(E.push("project = ?"),T.push(o)),i)if(Array.isArray(i)){let _=i.map(()=>"?").join(",");E.push(`type IN (${_})`),T.push(...i)}else E.push("type = ?"),T.push(i);if(a){let _=Array.isArray(a)?a:[a],R=_.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");T.push(..._),E.push(`(${R.join(" OR ")})`)}if(d){let _=Array.isArray(d)?d:[d],R=_.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");_.forEach(f=>{T.push(`%${f}%`,`%${f}%`)}),E.push(`(${R.join(" OR ")})`)}let g=E.length>0?`WHERE id IN (${l}) AND ${E.join(" AND ")}`:`WHERE id IN (${l})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${g}
    ...
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(m.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),p=new Date(d).toISOString();return this.db.transaction(()=>{let l=[],T=this.db.prepare(`
        ...
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let g of s){let S=T.run(e,t,g.type,g.title,g.subtitle,JSON.stringify(g.facts),g.narrative,JSON.stringify(g.concepts),JSON.stringify(g.files_read),JSON.stringify(g.files_modified),o||null,i,p,d);l.push(Number(S.lastInsertRowid))}let E=null;if(n){let S=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,p,d);E=Number(S.lastInsertRowid)}return{observationIds:l,summaryId:E,createdAtEpoch:d}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,p){let m=p??Date.now(),l=new Date(m).toISOString();return this.db.transaction(()=>{let E=[],g=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let R of s){let f=g.run(e,t,R.type,R.title,R.subtitle,JSON.stringify(R.facts),R.narrative,JSON.stringify(R.concepts),JSON.stringify(R.files_read),JSON.stringify(R.files_modified),a||null,d,l,m);E.push(Number(f.lastInsertRowid))}let S;if(n){let f=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,d,l,m);S=Number(f.lastInsertRowid)}return this.db.prepare(`
      ...
      `).run(m,o),{observationIds:E,summaryId:S,createdAtEpoch:m}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",d=e.map(()=>"?").join(","),p=[...e],m=o?`WHERE id IN (${d}) AND project = ?`:`WHERE id IN (${d})`;return o&&p.push(o),this.db.prepare(`
    ...
    `).all(...p)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],d,p;if(e!==null){let _=`
      ...
      `,R=`
        ...
        ORDER BY id ASC
        LIMIT ?
      `;try{let f=this.db.prepare(_).all(e,...a,s+1),h=this.db.prepare(R).all(e,...a,n+1);if(f.length===0&&h.length===0)return{observations:[],sessions:[],prompts:[]};d=f.length>0?f[f.length-1].created_at_epoch:t,p=h.length>0?h[h.length-1].created_at_epoch:t}catch(f){return u.error("DB","Error getting boundary observations",void 0,{error:f,project:o}),{observations:[],sessions:[],prompts:[]}}}else{let _=`
      ...
      `,R=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let f=this.db.prepare(_).all(t,...a,s),h=this.db.prepare(R).all(t,...a,n+1);if(f.length===0&&h.length===0)return{observations:[],sessions:[],prompts:[]};d=f.length>0?f[f.length-1].created_at_epoch:t,p=h.length>0?h[h.length-1].created_at_epoch:t}catch(f){return u.error("DB","Error getting boundary timestamps",void 0,{error:f,project:o}),{observations:[],sessions:[],prompts:[]}}}let m=`
      SELECT *
      FROM observations
...
`}function st(r,e,t,s){let n=[];return s?n.push(...Ge(r)):n.push(...Ce(r)),s?n.push(...He()):n.push(...ve()),s?n.push(...We()):n.push(...De()),s?n.push(...Ye()):n.push(...Me()),X(t)&&(s?n.push(...Ve(e,t)):n.push(...ke(e,t))),n}var se=y(require("path"),1);function W(r){if(!r)return[];try{let e=JSON.parse(r);return Array.isArray(e)?e:[]}catch(e){return u.debug("PARSER","Failed to parse JSON array, using empty fallback",{preview:r?.substring(0,50)},e),[]}}function nt(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}function ot(r){return new Date(r).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}function it(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric"})}function rt(r,e){return se.default.isAbsolute(r)?se.default.relative(e,r):r}function at(r,e,t){let s=W(r);if(s.length>0)return rt(s[0],e);if(t){let n=W(t);if(n.length>0)return rt(n[0],e)}return"General"}function Mt(r){let e=new Map;for(let s of r){let n=s.type==="observation"?s.data.created_at:s.data.displayTime,o=it(n);e.has(o)||e.set(o,[]),e.get(o).push(s)}let t=Array.from(e.entries()).sort((s,n)=>{let o=new Date(s[0]).getTime(),i=new Date(n[0]).getTime();return o-i});return new Map(t)}function kt(r,e){return e.fullObservationField==="narrative"?r.narrative:r.facts?W(r.facts).join(`
`):null}function xt(r,e,t,s,n,o){let i=[];o?i.push(...qe(r)):i.push(...xe(r));let a=null,d="",p=!1;for(let m of e)if(m.type==="summary"){p&&(i.push(""),p=!1,a=null,d="");let l=m.data,T=nt(l.displayTime);o?i.push(...Qe(l,T)):i.push(...Fe(l,T))}else{let l=m.data,T=at(l.files_modified,n,l.files_read),E=ot(l.created_at),g=E!==d,S=g?E:"";d=E;let _=t.has(l.id);if(T!==a&&(p&&i.push(""),o?i.push(...Ke(T)):i.push(...Ue(T)),a=T,p=!0),_){let R=kt(l,s);o?i.push(...ze(l,E,g,R,s)):(p&&!o&&(i.push(""),p=!1),i.push(...we(l,S,R,s)),a=null)}else o?i.push(Je(l,E,g,s)):i.push($e(l,S,s))}return p&&i.push(""),i}function dt(r,e,t,s,n){let o=[],i=Mt(r);for(let[a,d]of i)o.push(...xt(a,d,e,t,s,n));return o}function ct(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function pt(r,e){let t=[];return e?(t.push(...$("Investigated",r.investigated,c.blue)),t.push(...$("Learned",r.learned,c.yellow)),t.push(...$("Completed",r.completed,c.green)),t.push(...$("Next Steps",r.next_steps,c.magenta))):(t.push(...U("Investigated",r.investigated)),t.push(...U("Learned",r.learned)),t.push(...U("Completed",r.completed)),t.push(...U("Next Steps",r.next_steps))),t}function ut(r,e){return e?Ze(r):Pe(r)}function mt(r,e,t){return!X(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?et(r.totalDiscoveryTokens,r.totalReadTokens):Be(r.totalDiscoveryTokens,r.totalReadTokens)}function Ut(r,e){if(!r.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='tags'").get())return null;let s="AND o.project = ?",n=r.prepare(`
  ...
  `).get(e),d=r.prepare(`
    ...
    LIMIT 1
  `).get(e),p=null,m=null;if(d){p=d.title;let l=Date.now()-d.created_at_epoch,T=Math.floor(l/(1e3*60*60*24)),E=Math.floor(l/(1e3*60*60));T>0?m=`${T} day${T!==1?"s":""} ago`:E>0?m=`${E} hour${E!==1?"s":""} ago`:m="just now"}return{bugs:n.count,todos:o.count+i.count,learnings:a.count,lastDecision:p,lastDecisionAge:m}}function lt(r,e,t){let s=Ut(r,e);if(!s)return[];if(s.bugs===0&&s.todos===0&&s.learnings===0&&!s.lastDecision)return[];let n=[];if(n.push(""),n.push("## Project Status"),s.bugs>0&&n.push(`- ${s.bugs} open bug${s.bugs!==1?"s":""}`),s.todos>0&&n.push(`- ${s.todos} active todo${s.todos!==1?"s":""}`),s.learnings>0&&n.push(`- ${s.learnings} learning${s.learnings!==1?"s":""} captured`),s.lastDecision&&s.lastDecisionAge){let o=s.lastDecision.length>60?s.lastDecision.substring(0,57)+"...":s.lastDecision;n.push(`- Last decision: "${o}" (${s.lastDecisionAge})`)}try{let o=r.prepare(`
    ...
    `).get(e);if(o){let i=Date.now()-o.started_at_epoch,a=Math.floor(i/6e4),d=a<60?`${a} min ago`:`${Math.floor(a/60)}h ago`;n.push(`- Last session: ${d}, ${o.obs_count} observations`)}}catch{}try{let o=r.prepare(`
      ...
      ORDER BY created_at_epoch DESC
      LIMIT 5
    `).all(e),i=new Set;for(let a of o){try{for(let d of JSON.parse(a.files_modified))if(i.add(d),i.size>=5)break}catch{}if(i.size>=5)break}i.size>0&&n.push(`- Recent files: ${[...i].join(", ")}`)}catch{}return n}function _t(r,e,t){if(!r.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='loop_configs'").get())return[];let n=r.prepare("SELECT * FROM loop_configs WHERE project = ? AND enabled = 1").get(e);if(!n)return[];let o=r.prepare("SELECT * FROM loop_iterations WHERE loop_config_id = ? ORDER BY iteration_number ASC").all(n.id),i=[],a=o.length,d=n.task_description||"Unnamed task";if(i.push(""),i.push(`## Active Loop: "${d}" (Iteration ${a+1}/${n.max_iterations}, Mode: ${n.mode})`),o.length>0){i.push("### Previous Iterations:");for(let p of o.slice(-5)){let m=p.key_findings||"No findings recorded",l=p.observations_count>0?` [${p.observations_count} obs]`:"";i.push(`- Iter ${p.iteration_number} (${p.mode_used||n.mode}): ${m}${l}`)}}if(n.success_criteria&&(i.push("### Success Criteria:"),i.push(n.success_criteria)),n.completion_promises)try{let p=JSON.parse(n.completion_promises);if(p.length>0){let m=n.promise_logic==="all"?"ALL":"ANY";i.push(`### Completion (${m}): Say ${p.map(l=>`<promise>${l}</promise>`).join(" ")} when done.`)}}catch{}return i}var A=require("fs"),M=y(require("path"),1),re=require("os");function Et(r,e){let t=M.default.join((0,re.homedir)(),".claude","teams"),s=M.default.join((0,re.homedir)(),".claude","tasks");if(!(0,A.existsSync)(t))return[];let n=[],o=!1;try{let i=(0,A.readdirSync)(t,{withFileTypes:!0}).filter(a=>a.isDirectory()).map(a=>a.name);for(let a of i){let d=M.default.join(t,a,"config.json");if((0,A.existsSync)(d))try{let m=JSON.parse((0,A.readFileSync)(d,"utf-8")).members||[],l=0,T=0,E=M.default.join(s,a);if((0,A.existsSync)(E))try{let g=(0,A.readdirSync)(E).filter(S=>S.endsWith(".json"));for(let S of g)try{let _=JSON.parse((0,A.readFileSync)(M.default.join(E,S),"utf-8"));l++,(_.status==="completed"||_.status==="done")&&T++}catch{}}catch{}o||(n.push(""),n.push("## Team Context"),o=!0),n.push(`- Team "${a}": ${m.length} member${m.length!==1?"s":""}, ${T}/${l} tasks complete`)}catch{}}}catch{}return n}var $t=Tt.default.join((0,gt.homedir)(),".claude","plugins","marketplaces","EconLab-AI","plugin",".install-version");function wt(){try{return new P}catch(r){if(r.code==="ERR_DLOPEN_FAILED"){try{(0,ft.unlinkSync)($t)}catch(e){u.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return u.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function Ft(r,e){return e?tt(r):Xe(r)}function Pt(r,e,t,s,n,o,i,a){let d=[],p=z(e);d.push(...st(r,p,s,i));let m=t.slice(0,s.sessionCount),l=Ae(m,t),T=te(e,l),E=Le(e,s.fullObservationCount);if(d.push(...dt(T,E,s,n,i)),a){try{d.push(...lt(a.db,r,i))}catch{}try{d.push(..._t(a.db,r,i))}catch{}}try{d.push(...Et(r,i))}catch{}let g=t[0],S=e[0];ct(s,g,S)&&d.push(...pt(g,i));let _=ee(e,s,o,n);return d.push(...ut(_,i)),d.push(...mt(p,s,i)),d.join(`
`).trimEnd()}async function ne(r,e=!1){let t=q(),s=r?.cwd??process.cwd(),n=Re(s),o=r?.projects||[n],i=wt();if(!i)return"";try{let a=o.length>1?Ie(i,o,t):Q(i,n,t),d=o.length>1?Oe(i,o,t):Z(i,n,t);return a.length===0&&d.length===0?Ft(n,e):Pt(n,a,d,t,s,r?.session_id,e,i)}finally{i.close()}}0&&(module.exports={generateContext});
