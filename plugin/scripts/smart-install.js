#!/usr/bin/env node
/**
 * Smart Install Script for ultrabrain
 *
...
const ROOT = join(homedir(), '.claude', 'plugins', 'marketplaces', 'giulianofalco');
...
/**
 * Add shell alias for ultrabrain command
 */
function installCLI() {
  const WORKER_CLI = join(ROOT, 'plugin', 'scripts', 'worker-service.cjs');
  const bunPath = getBunPath() || 'bun';
  const aliasLine = `alias ultrabrain='${bunPath} "${WORKER_CLI}"'`;
  ...
  try {
    if (IS_WINDOWS) {
      ...
      const functionDef = `function ultrabrain { & "${bunPath}" "${WORKER_CLI}" $args }\n`;
      ...
      if (!existingContent.includes('function ultrabrain')) {
        writeFileSync(profilePath, existingContent + '\n' + functionDef);
        console.error(`✅ PowerShell function added to profile`);
        console.error('   Restart your terminal to use: ultrabrain <command>');
      }
    } else {
      ...
      for (const config of shellConfigs) {
        if (existsSync(config)) {
          ...
          if (!content.includes('alias ultrabrain=')) {
...
try {
  ...
  if (needsInstall()) {
    ...
    const port = process.env.ULTRABRAIN_WORKER_PORT || 37777;
    console.error(`[ultrabrain] Plugin updated to v${newVersion} - restarting worker...`);
    try {
