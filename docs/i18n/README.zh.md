...
<p align="center">
  UltraBrain 通过自动捕获工具使用观察、生成语义摘要并使其可用于未来会话,无缝保留跨会话的上下文。这使 Claude 能够在会话结束或重新连接后仍保持对项目的知识连续性。
</p>
...
📚 **[查看完整文档](https://docs.ultrabrain.ai/)** - 在官方网站浏览

### 入门指南

- **[安装指南](https://docs.ultrabrain.ai/installation)** - 快速开始与高级安装
- **[使用指南](https://docs.ultrabrain.ai/usage/getting-started)** - UltraBrain 如何自动工作
- **[搜索工具](https://docs.ultrabrain.ai/usage/search-tools)** - 使用自然语言查询项目历史
- **[测试版功能](https://docs.ultrabrain.ai/beta-features)** - 尝试实验性功能,如无尽模式

### 最佳实践

- **[上下文工程](https://docs.ultrabrain.ai/context-engineering)** - AI 代理上下文优化原则
- **[渐进式披露](https://docs.ultrabrain.ai/progressive-disclosure)** - UltraBrain 上下文启动策略背后的哲学

### 架构

- **[概述](https://docs.ultrabrain.ai/architecture/overview)** - 系统组件与数据流
- **[架构演进](https://docs.ultrabrain.ai/architecture-evolution)** - 从 v3 到 v5 的旅程
- **[钩子架构](https://docs.ultrabrain.ai/hooks-architecture)** - UltraBrain 如何使用生命周期钩子
- **[钩子参考](https://docs.ultrabrain.ai/architecture/hooks)** - 7 个钩子脚本详解
- **[Worker 服务](https://docs.ultrabrain.ai/architecture/worker-service)** - HTTP API 与 Bun 管理
- **[数据库](https://docs.ultrabrain.ai/architecture/database)** - SQLite 模式与 FTS5 搜索
- **[搜索架构](https://docs.ultrabrain.ai/architecture/search-architecture)** - 使用 Lance 向量数据库的混合搜索

### 配置与开发

- **[配置](https://docs.ultrabrain.ai/configuration)** - 环境变量与设置
- **[开发](https://docs.ultrabrain.ai/development)** - 构建、测试、贡献
- **[故障排除](https://docs.ultrabrain.ai/troubleshooting)** - 常见问题与解决方案
...
6. **Lance 向量数据库** - 混合语义 + 关键词搜索,实现智能上下文检索

详见[架构概述](https://docs.ultrabrain.ai/architecture/overview)。

---

## mem-search 技能

UltraBrain 通过 mem-search 技能提供智能搜索,当您询问过去的工作时会自动调用:
...
详见[搜索工具指南](https://docs.ultrabrain.ai/usage/search-tools)的详细示例。

---

## 测试版功能

UltraBrain 提供**测试版渠道**,包含实验性功能,如**无尽模式**(用于扩展会话的仿生记忆架构)。从 Web 查看器界面 http://localhost:37777 → 设置 切换稳定版和测试版。

详见 **[测试版功能文档](https://docs.ultrabrain.ai/beta-features)** 了解无尽模式的详细信息和试用方法。
...
设置在 `~/.ultrabrain/settings.json` 中管理(首次运行时自动创建默认设置)。可配置 AI 模型、worker 端口、数据目录、日志级别和上下文注入设置。

详见 **[配置指南](https://docs.ultrabrain.ai/configuration)** 了解所有可用设置和示例。

---

## 开发

详见 **[开发指南](https://docs.ultrabrain.ai/development)** 了解构建说明、测试和贡献工作流程。
...
详见 **[故障排除指南](https://docs.ultrabrain.ai/troubleshooting)** 了解常见问题和解决方案。
...
详见[开发指南](https://docs.ultrabrain.ai/development)了解贡献工作流程。

---

## 许可证

本项目采用 **MIT License** (MIT) 许可。

Copyright (c) 2026 Giuliano Falco (@giulianofalco)。保留所有权利。
...
- **文档**: [docs/](docs/)
- **问题反馈**: [GitHub Issues](https://github.com/EconLab-AI/Ultrabrain/issues)
- **仓库**: [github.com/EconLab-AI/Ultrabrain](https://github.com/EconLab-AI/Ultrabrain)
