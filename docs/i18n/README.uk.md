...
<p align="center">
  UltraBrain безперешкодно зберігає контекст між сесіями, автоматично фіксуючи спостереження за використанням інструментів, генеруючи семантичні резюме та роблячи їх доступними для майбутніх сесій. Це дозволяє Claude підтримувати безперервність знань про проєкти навіть після завершення або повторного підключення сесій.
</p>
...
📚 **[Переглянути повну документацію](https://docs.ultrabrain.ai/)** - Переглянути на офіційному сайті

### Початок роботи

- **[Посібник з встановлення](https://docs.ultrabrain.ai/installation)** - Швидкий старт і розширене встановлення
- **[Посібник з використання](https://docs.ultrabrain.ai/usage/getting-started)** - Як UltraBrain працює автоматично
- **[Інструменти пошуку](https://docs.ultrabrain.ai/usage/search-tools)** - Запитуйте історію свого проєкту природною мовою
- **[Бета-функції](https://docs.ultrabrain.ai/beta-features)** - Спробуйте експериментальні функції, як-от режим Endless Mode

### Найкращі практики

- **[Інженерія контексту](https://docs.ultrabrain.ai/context-engineering)** - Принципи оптимізації контексту AI-агента
- **[Прогресивне розкриття](https://docs.ultrabrain.ai/progressive-disclosure)** - Філософія стратегії підготовки контексту UltraBrain

### Архітектура

- **[Огляд](https://docs.ultrabrain.ai/architecture/overview)** - Компоненти системи та потік даних
- **[Еволюція архітектури](https://docs.ultrabrain.ai/architecture-evolution)** - Шлях від v3 до v5
- **[Архітектура хуків](https://docs.ultrabrain.ai/hooks-architecture)** - Як UltraBrain використовує хуки життєвого циклу
- **[Довідник хуків](https://docs.ultrabrain.ai/architecture/hooks)** - Пояснення 7 скриптів хуків
- **[Сервіс воркера](https://docs.ultrabrain.ai/architecture/worker-service)** - HTTP API та управління Bun
- **[База даних](https://docs.ultrabrain.ai/architecture/database)** - Схема SQLite та пошук FTS5
- **[Архітектура пошуку](https://docs.ultrabrain.ai/architecture/search-architecture)** - Гібридний пошук з векторною базою даних Chroma

### Конфігурація та розробка

- **[Конфігурація](https://docs.ultrabrain.ai/configuration)** - Змінні середовища та налаштування
- **[Розробка](https://docs.ultrabrain.ai/development)** - Збірка, тестування, внесок
- **[Усунення несправностей](https://docs.ultrabrain.ai/troubleshooting)** - Поширені проблеми та рішення
...
Дивіться [Огляд архітектури](https://docs.ultrabrain.ai/architecture/overview) для деталей.

---

## Навичка mem-search

UltraBrain надає інтелектуальний пошук через навичку mem-search, яка автоматично викликається, коли ви запитуєте про минулу роботу:
...
Дивіться [Посібник з інструментів пошуку](https://docs.ultrabrain.ai/usage/search-tools) для детальних прикладів.

---

## Бета-функції

UltraBrain пропонує **бета-канал** з експериментальними функціями, як-от **режим Endless Mode** (біоміметична архітектура пам'яті для тривалих сесій). Перемикайтеся між стабільною та бета-версіями з веб-інтерфейсу перегляду на http://localhost:37777 → Налаштування.

Дивіться **[Документацію бета-функцій](https://docs.ultrabrain.ai/beta-features)** для деталей про режим Endless Mode та як його спробувати.
...
Налаштування керуються в `~/.ultrabrain/settings.json` (автоматично створюється зі стандартними значеннями при першому запуску). Налаштуйте модель AI, порт воркера, каталог даних, рівень журналювання та параметри впровадження контексту.

Дивіться **[Посібник з конфігурації](https://docs.ultrabrain.ai/configuration)** для всіх доступних налаштувань та прикладів.

---

## Розробка

Дивіться **[Посібник з розробки](https://docs.ultrabrain.ai/development)** для інструкцій зі збірки, тестування та робочого процесу внеску.
...
Дивіться **[Посібник з усунення несправностей](https://docs.ultrabrain.ai/troubleshooting)** для поширених проблем та рішень.
...
Дивіться [Посібник з розробки](https://docs.ultrabrain.ai/development) для робочого процесу внеску.

---

## Ліцензія

Цей проєкт ліцензовано під **MIT License** (MIT).

Авторське право (C) 2025 Giuliano Falco (@giulianofalco). Всі права захищені.
...
- Похідні роботи також повинні бути ліцензовані під MIT
...
- **Проблеми**: [GitHub Issues](https://github.com/EconLab-AI/Ultrabrain/issues)
- **Репозиторій**: [github.com/EconLab-AI/Ultrabrain](https://github.com/EconLab-AI/Ultrabrain)
