...
description: Complete reference for integrating ultrabrain worker service into VSCode extensions, IDE plugins, and CLI tools
icon: plug
---

<Note>
**Version:** 7.0.0 (December 2025)
**Target Audience:** Developers building ultrabrain integrations (VSCode extensions, IDE plugins, CLI tools)
</Note>
...
const DEFAULT_PORT = 37777; // Override with ULTRABRAIN_WORKER_PORT
```
...
```bash
ULTRABRAIN_MODEL=claude-sonnet-4-5          # Model for observations/summaries
ULTRABRAIN_CONTEXT_OBSERVATIONS=50          # Observations injected at SessionStart
  ...
  → HTTP Request to Worker (localhost:37777)
    → Route Handler (SessionRoutes/DataRoutes/SearchRoutes/etc.)
      → Domain Service (SessionManager/SearchManager/DatabaseManager)
        → Database (SQLite3 + Lance vector DB)
...
**Tag Stripping:** Removes `<private>` and `<ultrabrain-context>` tags before storage.
...
```json Response
{
  "projects": ["ultrabrain", "other-project", ...]
}
```
...
// Register tool
vscode.lm.registerTool('ultrabrain-search', searchTool);
```

#### Chat Participant Implementation

```typescript
const participant = vscode.chat.createChatParticipant('ultrabrain', async (request, context, stream, token) => {
...
- `<ultrabrain-context>content</ultrabrain-context>` - System-level tag (prevents recursive storage)
...
class WorkerUnavailableError extends Error {
  constructor() {
    super('UltraBrain worker is not running or unreachable');
...
describe('Worker Integration', () => {
  ...
  beforeAll(async () => {
    ...
    workerProcess = spawn('node', ['dist/worker-service.js'], {
      env: { ...process.env, ULTRABRAIN_WORKER_PORT: '37778' }
    });
...
export function registerSearchTool(context: vscode.ExtensionContext) {
  ...
  const searchTool = vscode.lm.registerTool('ultrabrain-search', {
    description: 'Search persistent memory for observations, sessions, and prompts',
    inputSchema: {
      type: 'object',
      properties: {
...
<CardGroup cols={2}>
  <Card title="Documentation" icon="book" href="https://ultrabrain.ai">
    Complete ultrabrain documentation
  </Card>
  <Card title="GitHub" icon="github" href="https://github.com/EconLab-AI/Ultrabrain">
