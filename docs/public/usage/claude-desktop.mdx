...
description: Use ultrabrain memory search in Claude Desktop with MCP tools
icon: desktop
---

<Note>
**Availability:** UltraBrain MCP tools work with Claude Desktop on macOS and Windows.
</Note>

## Overview

Claude Desktop can access your ultrabrain memory database through **MCP tools**. This allows you to search past sessions, decisions, and observations directly from Claude Desktop conversations.

## Prerequisites

Before configuring MCP tools, ensure:

1. **ultrabrain is installed** and the worker service is running
    ...
    ```json
    {
      "mcpServers": {
        "mcp-search": {
          "command": "node",
          "args": [
            "/Users/YOUR_USERNAME/.claude/plugins/marketplaces/EconLab-AI/plugin/scripts/mcp-server.cjs"
          ]
        }
      }
    }
    ...
    ```json
    {
      "mcpServers": {
        "mcp-search": {
          "command": "node",
          "args": [
            "C:\\Users\\YOUR_USERNAME\\.claude\\plugins\\marketplaces\\EconLab-AI\\plugin\\scripts\\mcp-server.cjs"
          ]
        }
      }
    }
...
1. Ensure ultrabrain has recorded sessions (check http://localhost:37777)
2. Verify the database exists: `ls ~/.ultrabrain/ultrabrain.db`
