...
UltraBrain supports Google's Gemini API as an alternative to the Claude Agent SDK for extracting observations from your sessions. This can significantly reduce costs since Gemini offers a generous free tier.
...
|---------|--------|---------|-------------|
| `ULTRABRAIN_PROVIDER` | `claude`, `gemini` | `claude` | AI provider for observation extraction |
| `ULTRABRAIN_GEMINI_API_KEY` | string | — | Your Gemini API key |
| `ULTRABRAIN_GEMINI_MODEL` | `gemini-2.5-flash-lite`, `gemini-2.5-flash`, `gemini-3-flash-preview` | `gemini-2.5-flash-lite` | Gemini model to use |
| `ULTRABRAIN_GEMINI_BILLING_ENABLED` | `true`, `false` | `false` | Skip rate limiting if billing is enabled on Google Cloud |
...
```json
{
  ...
  "ULTRABRAIN_GEMINI_API_KEY": "your-api-key-here",
  "ULTRABRAIN_GEMINI_MODEL": "gemini-2.5-flash-lite",
  "ULTRABRAIN_GEMINI_BILLING_ENABLED": "true"
}
```
...
If Gemini is selected but encounters errors, ultrabrain automatically falls back to the Claude Agent SDK:
...
Either:
- Set `ULTRABRAIN_GEMINI_API_KEY` in `~/.ultrabrain/settings.json`, or
...
UltraBrain enforces these limits automatically with built-in delays between requests. Processing may be slower but stays within limits.
...
<Tip>
**Recommended**: Enable billing on your Google Cloud project to unlock much higher rate limits. You won't be charged unless you exceed the generous free quota. This allows ultrabrain to process observations instantly instead of waiting between requests.
</Tip>
