...
description: "Common issues and solutions for UltraBrain"
---
...
1. Check browser localStorage is enabled:
   ```javascript
   // In browser console
   localStorage.getItem('ultrabrain-settings')
   ```
...
3. Clear localStorage and try again:
   ```javascript
   // In browser console
   localStorage.removeItem('ultrabrain-settings')
   ```
...
**Symptoms**: Installation fails with lancedb or Python-related errors.
...
2. Install lancedb manually:
   ```bash
   cd ~/.claude/plugins/marketplaces/EconLab-AI
   npm install lancedb
   ```

3. Check lancedb health:
   ```bash
...
5. If Lance continues to fail, hybrid search will gracefully degrade to SQLite FTS5 only
...
1. Clear install cache:
   ```bash
   rm ~/.claude/plugins/marketplaces/EconLab-AI/.install-version
   ```
...
3. Check version marker:
   ```bash
   cat ~/.claude/plugins/marketplaces/EconLab-AI/.install-version
   cat ~/.claude/plugins/marketplaces/EconLab-AI/package.json | grep version
   ```
...
3. Check database for corruption:
   ```bash
   sqlite3 ~/.ultrabrain/ultrabrain.db "PRAGMA integrity_check;"
   ```
...
3. Verify database has observations:
   ```bash
   sqlite3 ~/.ultrabrain/ultrabrain.db "SELECT COUNT(*) FROM observations;"
   ```
...
3. **View stuck messages directly**:
   ```bash
   sqlite3 ~/.ultrabrain/ultrabrain.db "
...
1. Check if summaries exist:
   ```bash
   sqlite3 ~/.ultrabrain/ultrabrain.db "SELECT COUNT(*) FROM session_summaries;"
   ```
...
3. Check database size (large database = slow queries):
   ```bash
   ls -lh ~/.ultrabrain/ultrabrain.db
   ```

4. Optimize database:
   ```bash
   sqlite3 ~/.ultrabrain/ultrabrain.db "VACUUM;"
   ```
...
3. Check package.json exists:
   ```bash
   ls -la ~/.claude/plugins/marketplaces/EconLab-AI/package.json
   ```
...
2. Backup database:
   ```bash
   cp ~/.ultrabrain/ultrabrain.db ~/.ultrabrain/ultrabrain.db.backup
   ```
...
1. Check FTS5 tables exist:
   ```bash
   sqlite3 ~/.ultrabrain/ultrabrain.db "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '%_fts';"
   ```
...
3. Check triggers exist:
   ```bash
   sqlite3 ~/.ultrabrain/ultrabrain.db "SELECT name FROM sqlite_master WHERE type='trigger';"
   ```
...
2. Verify FTS5 tables populated:
   ```bash
   sqlite3 ~/.ultrabrain/ultrabrain.db "SELECT COUNT(*) FROM observations_fts;"
   ```
...
**Symptoms**: `/plugin install ultrabrain` fails.
...
3. Verify installation:
   ```bash
   ls -la ~/.claude/plugins/marketplaces/EconLab-AI/
   ```
...
**Solution**: Check `~/.ultrabrain/` exists and is writable.
...
2. **Create issue**: [GitHub Issues](https://github.com/EconLab-AI/Ultrabrain/issues)
...
- [Configuration](configuration) - Customize UltraBrain
