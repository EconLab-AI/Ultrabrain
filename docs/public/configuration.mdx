---
title: "Configuration"
description: "Environment variables and settings for UltraBrain"
---
...
|-------------------------------|---------------------------------|---------------------------------------|
| `ULTRABRAIN_MODEL`            | `sonnet`                        | AI model for processing observations (when using Claude) |
| `ULTRABRAIN_PROVIDER`         | `claude`                        | AI provider: `claude`, `gemini`, or `openrouter` |
| `ULTRABRAIN_MODE`             | `code`                          | Active mode profile (e.g., `code--es`, `email-investigation`) |
| `ULTRABRAIN_CONTEXT_OBSERVATIONS` | `50`                        | Number of observations to inject      |
| `ULTRABRAIN_WORKER_PORT`      | `37777`                         | Worker service port                   |
| `ULTRABRAIN_WORKER_HOST`      | `127.0.0.1`                     | Worker service host address           |
| `ULTRABRAIN_SKIP_TOOLS`       | `ListMcpResourcesTool,SlashCommand,Skill,TodoWrite,AskUserQuestion` | Comma-separated tools to exclude from observations |

### Gemini Provider Settings

| Setting                       | Default                         | Description                           |
|-------------------------------|---------------------------------|---------------------------------------|
| `ULTRABRAIN_GEMINI_API_KEY`   | —                               | Gemini API key ([get free key](https://aistudio.google.com/app/apikey)) |
| `ULTRABRAIN_GEMINI_MODEL`     | `gemini-2.5-flash-lite`          | Gemini model: `gemini-2.5-flash-lite`, `gemini-2.5-flash`, `gemini-3-flash-preview` |
...
| Setting                                      | Default                     | Description                           |
|----------------------------------------------|-----------------------------|---------------------------------------|
| `ULTRABRAIN_OPENROUTER_API_KEY`              | —                           | OpenRouter API key ([get key](https://openrouter.ai/keys)) |
| `ULTRABRAIN_OPENROUTER_MODEL`                | `xiaomi/mimo-v2-flash:free` | Model identifier (supports 100+ models) |
| `ULTRABRAIN_OPENROUTER_MAX_CONTEXT_MESSAGES` | `20`                        | Max messages in conversation history  |
| `ULTRABRAIN_OPENROUTER_MAX_TOKENS`           | `100000`                    | Token budget safety limit             |
| `ULTRABRAIN_OPENROUTER_SITE_URL`             | —                           | Optional: URL for analytics           |
| `ULTRABRAIN_OPENROUTER_APP_NAME`             | `ultrabrain`                | Optional: App name for analytics      |
...
| Setting                       | Default                         | Description                           |
|-------------------------------|---------------------------------|---------------------------------------|
| `ULTRABRAIN_DATA_DIR`         | `~/.ultrabrain`                 | Data directory location               |
| `ULTRABRAIN_LOG_LEVEL`        | `INFO`                          | Log verbosity (DEBUG, INFO, WARN, ERROR, SILENT) |
| `ULTRABRAIN_PYTHON_VERSION`   | `3.13`                          | Python version for lancedb         |
...
```bash
./ultrabrain-settings.sh
```

This script manages settings in `~/.ultrabrain/settings.json`.

### Manual Configuration

Edit `~/.ultrabrain/settings.json`:

```json
{
  "ULTRABRAIN_MODEL": "sonnet"
}
```
...
| Setting | Default | Description |
|---------|---------|-------------|
| `ULTRABRAIN_MODE` | `code` | Defines behavior and language. See [Modes & Languages](modes). |
...
```json
{
  "ULTRABRAIN_MODE": "email-investigation"
}
```
...
- **Production (installed plugin)**: `~/.ultrabrain/` (always, regardless of CLAUDE_PLUGIN_ROOT)
- **Development**: Can be overridden with `ULTRABRAIN_DATA_DIR`

```
~/.ultrabrain/
├── ultrabrain.db           # SQLite database
...
```json
{
  "description": "UltraBrain memory system hooks",
  "hooks": {
    "SessionStart": [{
      "hooks": [{
        "type": "command",
...
UltraBrain provides MCP search tools for querying your project history.
...
UltraBrain supports switching between stable and beta versions via the web viewer UI.
...
UltraBrain can automatically generate `CLAUDE.md` files in your project folders with activity timelines. This feature is disabled by default.

| Setting | Default | Description |
|---------|---------|-------------|
| `ULTRABRAIN_FOLDER_CLAUDEMD_ENABLED` | `false` | Enable auto-generation of folder CLAUDE.md files |
...
UltraBrain injects past observations into each new session, giving Claude awareness of recent work. You can configure exactly what gets injected using the **Context Settings Modal**.
...
```json
{
  "ULTRABRAIN_CONTEXT_OBSERVATIONS": "100",
  "ULTRABRAIN_CONTEXT_SESSION_COUNT": "20",
  "ULTRABRAIN_CONTEXT_OBSERVATION_TYPES": "bugfix,decision,discovery",
  "ULTRABRAIN_CONTEXT_OBSERVATION_CONCEPTS": "how-it-works,gotcha",
  "ULTRABRAIN_CONTEXT_FULL_COUNT": "10",
  ...
  "ULTRABRAIN_CONTEXT_SHOW_LAST_SUMMARY": "false",
...
Settings can be customized in `~/.ultrabrain/settings.json`.
...
```json
{
  "ULTRABRAIN_DATA_DIR": "/custom/path"
}
```
...
```json
{
  "ULTRABRAIN_MODEL": "opus"
}
```

Then restart the worker:
```bash
export ULTRABRAIN_MODEL=opus
npm run worker:restart
```

### Custom Skip Tools

Control which tools are excluded from observations. Edit `~/.ultrabrain/settings.json`:
```json
{
  "ULTRABRAIN_SKIP_TOOLS": "ListMcpResourcesTool,SlashCommand,Skill"
}
```
...
2. Verify environment variables:
   ```bash
   echo $ULTRABRAIN_MODEL
   echo $ULTRABRAIN_WORKER_PORT
   ```
...
1. Set custom port:
   ```bash
   export ULTRABRAIN_WORKER_PORT=38000
   ```
