...
description: "How ultrabrain evolved from v3 to v5+"
---
...
This is the story of how ultrabrain evolved from a simple idea to a production-ready system, and the key architectural decisions that made it work.
...
const ThemeProvider = ({ children }) => {
  const [theme, setTheme] = useState<'light' | 'dark' | 'system'>(() => {
    return localStorage.getItem('ultrabrain-theme') || 'system';
  });

  useEffect(() => {
    localStorage.setItem('ultrabrain-theme', theme);
  }, [theme]);
...
**Impact**: Cross-platform compatibility restored, Windows users can now use ultrabrain without issues.
...
**The Evolution**: SQLite FTS5 + Lance vector search
...
- Python 3.8+ - Required by lancedb
...
```bash
cp ~/.ultrabrain/ultrabrain.db ~/.ultrabrain/ultrabrain-v3-backup.db
```
...
```bash
/plugin update ultrabrain
```

**What happens automatically:**
- Dependencies update (including new ones like lancedb for v5.0.0+)
