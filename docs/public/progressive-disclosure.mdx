# Progressive Disclosure: UltraBrain's Context Priming Philosophy
...
## UltraBrain's Solution: Progressive Disclosure
...
```markdown
*Use ultrabrain MCP search to access records with the given ID*
```
...
UltraBrain implements progressive disclosure through a 3-layer workflow pattern:
...
- [UltraBrain Architecture](architecture/overview) - How it all fits together
...
*This philosophy emerged from real-world usage of UltraBrain across hundreds of coding sessions. The pattern works because it aligns with both human cognition and LLM attention mechanics.*
