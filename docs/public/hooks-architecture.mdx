# How UltraBrain Uses Hooks: A Lifecycle-Driven Architecture
...
UltraBrain is fundamentally a **hook-driven system**. Every piece of functionality happens in response to lifecycle events:
...
**Key insight:** UltraBrain doesn't interrupt or modify Claude Code's behavior. It observes from the outside and provides value through lifecycle hooks.
...
UltraBrain had several architectural constraints:
...
UltraBrain uses lifecycle hook scripts across 5 lifecycle events. SessionStart runs 3 hooks in sequence: smart-install, worker-service start, and context-hook.
...
- ✅ Configurable observation count via `ULTRABRAIN_CONTEXT_OBSERVATIONS`

**Output format:**
```markdown
# [ultrabrain] recent context
...
<AccordionGroup>
  ...
  <Accordion title="Observations not captured">
    ...
    **Debugging:**
    1. Check database: `sqlite3 ~/.ultrabrain/ultrabrain.db "SELECT * FROM observation_queue"`
...
**Privacy guarantees:**
- All data stored locally in `~/.ultrabrain/ultrabrain.db`
...
*The hook-driven architecture enables UltraBrain to be both powerful and invisible. Users never notice the memory system working - it just makes Claude smarter over time.*
