...
description: "Configure UltraBrain behavior and language with the Mode System"
---

# Modes & Languages

UltraBrain uses a flexible **Mode System** to adapt its behavior, observation types, and output language. This allows you to switch between different workflows (like coding vs. email investigation) or languages without reinstalling the plugin.
...
Set the active mode using the `ULTRABRAIN_MODE` setting in `~/.ultrabrain/settings.json`:
...
```bash
export ULTRABRAIN_MODE="code--fr"
```
