...
This document describes the PM2 to Bun migration that occurred in v7.1.0 (December 2025). If you're installing ultrabrain for the first time, this migration has already been completed and you can use the current Bun-based system documented in the main guides.
...
UltraBrain version 7.1.0 introduces two major architectural migrations:
...
- **Process Name**: `ultrabrain-worker`
...
- **Logs**: `~/.pm2/logs/ultrabrain-worker-*.log`
- **PID File**: `~/.pm2/pids/ultrabrain-worker.pid`

**Lifecycle Commands**:
```bash
pm2 start <script>           # Start worker
pm2 stop ultrabrain-worker   # Stop worker
pm2 restart ultrabrain-worker # Restart worker
pm2 delete ultrabrain-worker  # Remove from PM2
pm2 logs ultrabrain-worker    # View logs
```
...
- **Logs**: `~/.ultrabrain/logs/worker-YYYY-MM-DD.log`
- **PID File**: `~/.ultrabrain/.worker.pid`
- **Port File**: `~/.ultrabrain/.worker.port` (new)
...
- Database file: `~/.ultrabrain/ultrabrain.db` (same location)
...
if (!existsSync(pm2MigratedMarker)) {
  try {
    spawnSync('pm2', ['delete', 'ultrabrain-worker'], { stdio: 'ignore' });
...
`ensureWorkerRunning()` checks if `~/.ultrabrain/.worker.pid` exists (it doesn't for first run after update)
</Step>
<Step title="Start Worker Decision">
Worker not running → Call `startWorker()`
</Step>
<Step title="Migration Check">
Check if `~/.ultrabrain/.pm2-migrated` exists
</Step>
<Step title="PM2 Cleanup">
Execute `pm2 delete ultrabrain-worker` (errors ignored), create marker file
</Step>
...
**Location**: `~/.ultrabrain/.pm2-migrated`
...
4. **PM2 cleanup**: `pm2 delete ultrabrain-worker` (old worker terminated)
5. **Marker creation**: `~/.ultrabrain/.pm2-migrated` with timestamp
...
| `pm2 stop ultrabrain-worker` | `npm run worker:stop` | Stop worker |
| `pm2 restart ultrabrain-worker` | `npm run worker:restart` | Restart worker |
| `pm2 delete ultrabrain-worker` | `npm run worker:stop` | Remove worker |
| `pm2 logs ultrabrain-worker` | `npm run worker:logs` | View logs |
| `pm2 describe ultrabrain-worker` | `npm run worker:status` | Detailed status |
...
**Logs**:
```
Old: ~/.pm2/logs/ultrabrain-worker-out.log
     ~/.pm2/logs/ultrabrain-worker-error.log

New: ~/.ultrabrain/logs/worker-YYYY-MM-DD.log
```

**PID Files**:
```
Old: ~/.pm2/pids/ultrabrain-worker.pid

New: ~/.ultrabrain/.worker.pid
```
...
New: ~/.ultrabrain/.worker.pid
     ~/.ultrabrain/.worker.port
     ~/.ultrabrain/.pm2-migrated (all platforms)
```

**Database** (unchanged):
```
Same: ~/.ultrabrain/ultrabrain.db
```
...
├────┼────────────────────┼─────────┼─────────┼──────────┤
│ 0  │ ultrabrain-worker  │ online  │ 0       │ 2d 5h    │
└────┴────────────────────┴─────────┴─────────┴──────────┘
```
...
# Or just remove ultrabrain logs
rm -f ~/.pm2/logs/ultrabrain-worker-*.log
rm -f ~/.pm2/pids/ultrabrain-worker.pid
```
...
```
~/.ultrabrain/
├── ultrabrain.db          # Database (unchanged)
...
~/.pm2/
├── logs/
│   ├── ultrabrain-worker-out.log
│   └── ultrabrain-worker-error.log
├── pids/
│   └── ultrabrain-worker.pid
└── pm2.log
```
...
```
~/.ultrabrain/
├── ultrabrain.db          # Database (same file)
├── chroma/                # Vector embeddings (unchanged)
├── logs/
...
**Symptoms**:
- `pm2 list` still shows `ultrabrain-worker`
...
**Manual Resolution**:
```bash
rm ~/.ultrabrain/.worker.pid
rm ~/.ultrabrain/.worker.port
...
| `Invalid port X` | Port validation failed | Update `ULTRABRAIN_WORKER_PORT` in settings |
...
# 2. Start PM2 worker
pm2 start plugin/scripts/worker-cli.js --name ultrabrain-worker
...
# 5. Verify migration
pm2 list  # Should NOT show ultrabrain-worker
cat ~/.ultrabrain/.pm2-migrated  # Should exist
