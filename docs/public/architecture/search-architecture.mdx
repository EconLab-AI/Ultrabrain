...
UltraBrain uses an **MCP-based search architecture** that provides intelligent memory retrieval through 4 streamlined tools following a 3-layer workflow pattern.
...
6. **Lance Vector DB** - Semantic search with hybrid retrieval
...
**Location:** `/Users/YOUR_USERNAME/.claude/plugins/marketplaces/EconLab-AI/plugin/scripts/mcp-server.cjs`
...
- Hybrid search with LanceDB for semantic similarity
...
The 3-layer workflow embodies **progressive disclosure** - a core principle of ultrabrain's architecture.
...
2. Check database: `ls ~/.ultrabrain/ultrabrain.db`
