...
- **Port**: Fixed port 37777 (configurable via `ULTRABRAIN_WORKER_PORT`)
- **Location**: `src/services/worker-service.ts`
- **Built Output**: `plugin/scripts/worker-service.cjs`
- **Model**: Configurable via `ULTRABRAIN_MODEL` environment variable (default: sonnet)
...
- PID file tracking at `~/.ultrabrain/worker.pid`
...
```bash
export ULTRABRAIN_MODEL=sonnet
```
...
- **Default**: Port 37777
- **Override**: Set `ULTRABRAIN_WORKER_PORT` environment variable
...
```
~/.ultrabrain/
├── ultrabrain.db           # SQLite database (bun:sqlite)
