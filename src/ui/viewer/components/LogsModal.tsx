...
type LogComponent = 'HOOK' | 'WORKER' | 'SDK' | 'PARSER' | 'DB' | 'SYSTEM' | 'HTTP' | 'SESSION' | 'LANCE';
...
const LOG_COMPONENTS: { key: LogComponent; label: string; icon: string; color: string }[] = [
  ...
  { key: 'LANCE', label: 'Lance', icon: '🔮', color: '#a855f7' },
];
...
export function LogsDrawer({ isOpen, onClose }: LogsDrawerProps) {
  ...
  const [activeComponents, setActiveComponents] = useState<Set<LogComponent>>(
    new Set(['HOOK', 'WORKER', 'SDK', 'PARSER', 'DB', 'SYSTEM', 'HTTP', 'SESSION', 'LANCE'])
  );
  ...
  const setAllComponents = useCallback((enabled: boolean) => {
    if (enabled) {
      setActiveComponents(new Set(['HOOK', 'WORKER', 'SDK', 'PARSER', 'DB', 'SYSTEM', 'HTTP', 'SESSION', 'LANCE']));
    } else {
