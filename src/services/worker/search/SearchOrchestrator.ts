...
import { LanceDbSearchStrategy } from './strategies/LanceDbSearchStrategy.js';
...
export class SearchOrchestrator {
  private lanceStrategy: LanceDbSearchStrategy | null = null;
  ...
  constructor(
    ...
    private lanceSync: LanceSync | null
  ) {
    // Initialize strategies
    this.sqliteStrategy = new SQLiteSearchStrategy(sessionSearch);

    if (lanceSync) {
      this.lanceStrategy = new LanceDbSearchStrategy(lanceSync, sessionStore);
      this.hybridStrategy = new HybridSearchStrategy(lanceSync, sessionStore, sessionSearch);
    }
  ...
  private async executeWithFallback(
    ...
    if (this.lanceStrategy) {
      logger.debug('SEARCH', 'Orchestrator: Using LanceDB semantic search', {});
      const result = await this.lanceStrategy.search(options);

      // If LanceDB succeeded (even with 0 results), return
      if (result.usedLance) {
        return result;
      }

      // LanceDB failed - fall back to SQLite for filter-only
      logger.debug('SEARCH', 'Orchestrator: LanceDB failed, falling back to SQLite', {});
    ...
    // PATH 3: No LanceDB available
    logger.debug('SEARCH', 'Orchestrator: LanceDB not available', {});
    return {
  ...
  async findByFile(filePath: string, args: any): Promise<{
    ...
    return { ...results, usedLance: false };
  }
  ...
  formatSearchResults(
    ...
    query: string,
    lanceFailed: boolean = false
  ): string {
    return this.resultFormatter.formatSearchResults(results, query, lanceFailed);
  }
