 ...
 * 2. LanceDB semantic ranking (rank by relevance)
 * 3. Intersection (keep only IDs from step 1, in rank order from step 2)
 * 4. Hydrate from SQLite in semantic rank order
 *
 * Used for: findByConcept, findByFile, findByType with LanceDB available
 */
...
export class HybridSearchStrategy extends BaseSearchStrategy implements SearchStrategy {
  ...
  canHandle(options: StrategySearchOptions): boolean {
    // Can handle when we have metadata filters and Lance is available
  ...
  async search(options: StrategySearchOptions): Promise<StrategySearchResult> {
    ...
    // For generic hybrid search, use the standard Lance path
   ...
   * Pattern: Metadata filter -> Lance ranking -> Intersection -> Hydrate
   */
  async findByConcept(
    concept: string,
    ...
    try {
      ...
      // Step 2: Lance semantic ranking
      const ids = metadataResults.map(obs => obs.id);
      const lanceResults = await this.lanceSync.queryLance(
        concept,
        Math.min(ids.length, SEARCH_CONSTANTS.LANCE_BATCH_SIZE)
      );

      // Step 3: Intersect - keep only IDs from metadata, in Lance rank order
      const rankedIds = this.intersectWithRanking(ids, lanceResults.ids);
  ...
  async findByFile(
    ...
    try {
      ...
      if (metadataResults.observations.length === 0) {
        return { observations: [], sessions, usedLance: false };
      }

      // Step 2: Lance semantic ranking for observations
      ...
      if (rankedIds.length > 0) {
        ...
        return { observations, sessions, usedLance: true };
      }
  ...
  /**
   * Intersect metadata IDs with Lance IDs, preserving Lance's rank order
   */
  private intersectWithRanking(metadataIds: number[], lanceIds: number[]): number[] {
    const metadataSet = new Set(metadataIds);
    const rankedIds: number[] = [];

    for (const lanceId of lanceIds) {
      if (metadataSet.has(lanceId) && !rankedIds.includes(lanceId)) {
