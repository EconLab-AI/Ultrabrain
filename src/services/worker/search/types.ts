...
/**
 * Document types stored in Lance
 */
export type LanceDocType = 'observation' | 'session_summary' | 'user_prompt';

/**
 * Lance query result with typed metadata
 */
export interface LanceQueryResult {
  ids: number[];
  distances: number[];
...
/**
 * Metadata stored with each Lance document
 */
export interface LanceMetadata {
  sqlite_id: number;
  doc_type: LanceDocType;
 ...
 */
export type SearchStrategyHint = 'lance' | 'sqlite' | 'hybrid' | 'auto';
...
export interface StrategySearchResult {
  ...
  /** Whether Lance was used successfully */
  usedLance: boolean;
