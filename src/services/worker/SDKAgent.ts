...
export class SDKAgent {
  ...
  async startSession(session: ActiveSession, worker?: WorkerRef): Promise<void> {
    ...
    // Build isolated environment from ~/.ultrabrain/.env
    ...
    const queryResult = query({
      ...
      options: {
        ...
        env: isolatedEnv  // Use isolated credentials from ~/.ultrabrain/.env, not process.env
      }
    });
  ...
  private findClaudeExecutable(): string {
    ...
    throw new Error('Claude executable not found. Please either:\n1. Add "claude" to your system PATH, or\n2. Set CLAUDE_CODE_PATH in ~/.ultrabrain/settings.json');
  }
  ...
  private getModelId(): string {
    ...
    return settings.ULTRABRAIN_MODEL;
  }
}
