...
export class DatabaseManager {
  ...
  private lanceSync: LanceSync | null = null;
  ...
  async initialize(): Promise<void> {
    ...
    // Initialize LanceSync (lazy - connects on first search, not at startup)
    this.lanceSync = new LanceSync('ultrabrain');
  ...
  async close(): Promise<void> {
    // Close LanceSync first
    if (this.lanceSync) {
      await this.lanceSync.close();
      this.lanceSync = null;
    }
  ...
  /**
   * Get LanceSync instance (throws if not initialized)
   */
  getLanceSync(): LanceSync {
    if (!this.lanceSync) {
      throw new Error('LanceSync not initialized');
    }
