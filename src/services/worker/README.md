...
The Worker Service is an Express HTTP server that handles all ultrabrain operations. It runs on port 37777 (configurable via `ULTRABRAIN_WORKER_PORT`) and is managed by PM2.
  ...
  → HTTP Request to Worker (localhost:37777)
    → Route Handler (http/routes/*.ts)
      → MCP Server Tool (for search) OR Service Layer (for session/data)
        → Database (SQLite3 + LanceDB vector DB)
```
