...
export class GeminiAgent {
  ...
  async startSession(session: ActiveSession, worker?: WorkerRef): Promise<void> {
    try {
      ...
      if (!apiKey) {
        throw new Error('Gemini API key not configured. Set ULTRABRAIN_GEMINI_API_KEY in settings or GEMINI_API_KEY environment variable.');
      }
  ...
  private getGeminiConfig(): { apiKey: string; model: GeminiModel; rateLimitingEnabled: boolean } {
    ...
    const apiKey = settings.ULTRABRAIN_GEMINI_API_KEY || getCredential('GEMINI_API_KEY') || '';

    // Model: from settings or default, with validation
    const defaultModel: GeminiModel = 'gemini-2.5-flash';
    const configuredModel = settings.ULTRABRAIN_GEMINI_MODEL || defaultModel;
    ...
    const rateLimitingEnabled = settings.ULTRABRAIN_GEMINI_RATE_LIMITING_ENABLED !== 'false';
...
export function isGeminiAvailable(): boolean {
  const settingsPath = path.join(homedir(), '.ultrabrain', 'settings.json');
  const settings = SettingsDefaultsManager.loadFromFile(settingsPath);
  return !!(settings.ULTRABRAIN_GEMINI_API_KEY || getCredential('GEMINI_API_KEY'));
}
...
export function isGeminiSelected(): boolean {
  const settingsPath = path.join(homedir(), '.ultrabrain', 'settings.json');
  const settings = SettingsDefaultsManager.loadFromFile(settingsPath);
  return settings.ULTRABRAIN_PROVIDER === 'gemini';
}
