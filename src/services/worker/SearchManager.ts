/**
 * SearchManager - Core search orchestration for ultrabrain
 *
 ...
 * - LanceDbSearchStrategy: Vector-based semantic search
...
export class SearchManager {
  ...
  /**
   * Query LanceDB vector database via LanceSync
   * @deprecated Use orchestrator.search() instead
   */
  private async queryLance(
    query: string,
    limit: number,
    whereFilter?: Record<string, any>
  ): Promise<{ ids: number[]; distances: number[]; metadatas: any[] }> {
    return await this.lanceSync.queryLance(query, limit, whereFilter);
  }
  ...
  async search(args: any): Promise<any> {
    ...
    // This path enables date filtering which vector search cannot do (requires direct SQLite access)
    if (!query) {
    ...
    // PATH 2: LANCE SEMANTIC SEARCH (query text + LanceDB available)
    else if (this.lanceSync) {
      let lanceSucceeded = false;
      logger.debug('SEARCH', 'Using LanceDB semantic search', { typeFilter: type || 'all' });
      ...
      // Step 1: LanceDB semantic search with optional type filter
      const lanceResults = await this.queryLance(query, 100, whereFilter);
      lanceSucceeded = true; // LanceDB didn't throw error
      logger.debug('SEARCH', 'LanceDB returned semantic matches', { matchCount: lanceResults.ids.length });

      if (lanceResults.ids.length > 0) {
        // Step 2: Filter by recency (90 days)
        const ninetyDaysAgo = Date.now() - SEARCH_CONSTANTS.RECENCY_WINDOW_MS;
        const recentMetadata = lanceResults.metadatas.map((meta, idx) => ({
          id: lanceResults.ids[idx],
          meta,
        ...
        // LanceDB returned 0 results - this is the correct answer, don't fall back to FTS5
        logger.debug('SEARCH', 'LanceDB found no matches (final result, no FTS5 fallback)', {});
      }
    }
    // LanceDB not initialized - mark as failed to show proper error message
    else if (query) {
      lanceFailed = true;
      logger.debug('SEARCH', 'LanceDB not initialized - semantic search unavailable', {});
      observations = [];
      sessions = [];
      prompts = [];
    }
  ...
  async timeline(args: any): Promise<any> {
    ...
    if (query) {
      ...
      if (this.lanceSync) {
        try {
          logger.debug('SEARCH', 'Using LanceDB semantic search for timeline query', {});
          const lanceResults = await this.queryLance(query, 100);
          logger.debug('SEARCH', 'LanceDB returned semantic matches for timeline', { matchCount: lanceResults?.ids?.length ?? 0 });

          if (lanceResults?.ids && lanceResults.ids.length > 0) {
            const ninetyDaysAgo = Date.now() - SEARCH_CONSTANTS.RECENCY_WINDOW_MS;
            const recentIds = lanceResults.ids.filter((_id, idx) => {
              const meta = lanceResults.metadatas[idx];
          ...
          logger.error('SEARCH', 'LanceDB search failed for timeline, continuing without semantic results', {}, lanceError as Error);
        }
      }
    ...
    if (this.lanceSync) {
      try {
        if (query) {
          ...
          logger.debug('SEARCH', 'Using LanceDB semantic search with type=decision filter', {});
          const lanceResults = await this.queryLance(query, Math.min((filters.limit || 20) * 2, 100), { type: 'decision' });
          const obsIds = lanceResults.ids;

          if (obsIds.length > 0) {
            results = this.sessionStore.getObservationsByIds(obsIds, { ...filters, type: 'decision' });
            // Preserve LanceDB ranking order
          ...
          if (metadataResults.length > 0) {
            ...
            const lanceResults = await this.queryLance('decision', Math.min(ids.length, 100));

            const rankedIds: number[] = [];
            for (const lanceId of lanceResults.ids) {
              if (ids.includes(lanceId) && !rankedIds.includes(lanceId)) {
                rankedIds.push(lanceId);
              }
            }
        ...
        logger.error('SEARCH', 'LanceDB search failed for decisions, falling back to metadata search', {}, lanceError as Error);
      }
    }
  ...
  async changes(args: any): Promise<any> {
    ...
    if (this.lanceSync) {
      try {
        logger.debug('SEARCH', 'Using LanceDB search for change-related observations', {});
        ...
        if (allIds.size > 0) {
          ...
          const lanceResults = await this.queryLance('what changed', Math.min(idsArray.length, 100));

          const rankedIds: number[] = [];
          for (const lanceId of lanceResults.ids) {
            if (idsArray.includes(lanceId) && !rankedIds.includes(lanceId)) {
              rankedIds.push(lanceId);
            }
          }
        ...
        logger.error('SEARCH', 'LanceDB search failed for changes, falling back to metadata search', {}, lanceError as Error);
      }
    }
  ...
  async howItWorks(args: any): Promise<any> {
    ...
    if (this.lanceSync) {
      ...
      if (metadataResults.length > 0) {
        const ids = metadataResults.map(obs => obs.id);
        const lanceResults = await this.queryLance('how it works architecture', Math.min(ids.length, 100));
  ...
  async searchObservations(args: any): Promise<any> {
    ...
    // Vector-first search via LanceDB
    if (this.lanceSync) {
      logger.debug('SEARCH', 'Using hybrid semantic search (LanceDB + SQLite)', {});

      // Step 1: LanceDB semantic search (top 100)
      const lanceResults = await this.queryLance(query, 100);
      logger.debug('SEARCH', 'LanceDB returned semantic matches', { matchCount: lanceResults.ids.length });
  ...
  async searchSessions(args: any): Promise<any> {
    ...
    // Vector-first search via LanceDB
    if (this.lanceSync) {
      logger.debug('SEARCH', 'Using LanceDB semantic search for sessions', {});

      // Step 1: LanceDB semantic search (top 100)
      const lanceResults = await this.queryLance(query, 100, { doc_type: 'session_summary' });
      logger.debug('SEARCH', 'LanceDB returned semantic matches for sessions', { matchCount: lanceResults.ids.length });
  ...
  async searchUserPrompts(args: any): Promise<any> {
    ...
    // Vector-first search via LanceDB
    if (this.lanceSync) {
      logger.debug('SEARCH', 'Using LanceDB semantic search for user prompts', {});

      // Step 1: LanceDB semantic search (top 100)
      const lanceResults = await this.queryLance(query, 100, { doc_type: 'user_prompt' });
      logger.debug('SEARCH', 'LanceDB returned semantic matches for prompts', { matchCount: lanceResults.ids.length });
  ...
  async findByConcept(args: any): Promise<any> {
    ...
    if (this.lanceSync) {
      ...
      if (metadataResults.length > 0) {
        // Step 2: LanceDB semantic ranking (rank by relevance to concept)
        const ids = metadataResults.map(obs => obs.id);
        const lanceResults = await this.queryLance(concept, Math.min(ids.length, 100));
        ...
        logger.debug('SEARCH', 'LanceDB ranked results by semantic relevance', { count: rankedIds.length });
    ...
    // Fall back to SQLite-only if LanceDB unavailable or failed
    if (results.length === 0) {
  ...
  async findByFile(args: any): Promise<any> {
    ...
    if (this.lanceSync) {
      ...
      if (metadataResults.observations.length > 0) {
        // Step 2: LanceDB semantic ranking (rank by relevance to file path)
        const ids = metadataResults.observations.map(obs => obs.id);
        const lanceResults = await this.queryLance(filePath, Math.min(ids.length, 100));
        ...
        logger.debug('SEARCH', 'LanceDB ranked observations by semantic relevance', { count: rankedIds.length });
  ...
  async findByType(args: any): Promise<any> {
    ...
    if (this.lanceSync) {
      ...
      if (metadataResults.length > 0) {
        // Step 2: LanceDB semantic ranking (rank by relevance to type)
        const ids = metadataResults.map(obs => obs.id);
        const lanceResults = await this.queryLance(typeStr, Math.min(ids.length, 100));
  ...
  async getTimelineByQuery(args: any): Promise<any> {
    ...
    if (this.lanceSync) {
      logger.debug('SEARCH', 'Using LanceDB semantic search for timeline query', {});
      const lanceResults = await this.queryLance(query, 100);
      logger.debug('SEARCH', 'LanceDB returned semantic matches for timeline', { matchCount: lanceResults.ids.length });
