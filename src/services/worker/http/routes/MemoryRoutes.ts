...
export class MemoryRoutes extends BaseRouteHandler {
  ...
  private handleSaveMemory = this.wrapHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    const lanceSync = this.dbManager.getLanceSync();
    ...
    // 4. Sync to LanceDB (async, fire-and-forget)
    lanceSync.syncObservation(
      result.id,
      memorySessionId,
      targetProject,
      observation,
      ...
      logger.error('LANCE', 'LanceDB sync failed', { id: result.id }, err as Error);
    });
