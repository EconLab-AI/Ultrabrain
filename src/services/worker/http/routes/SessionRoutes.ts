...
import { GroqAgent, isGroqSelected, isGroqAvailable } from '../../GroqAgent.js';
...
export class SessionRoutes extends BaseRouteHandler {
   ...
   */
  private getActiveAgent(): SDKAgent | GeminiAgent | OpenRouterAgent | GroqAgent {
    if (isGroqSelected()) {
      if (isGroqAvailable()) {
        logger.debug('SESSION', 'Using Groq agent');
        return this.groqAgent;
      } else {
        throw new Error('Groq provider selected but no API key configured. Set ULTRABRAIN_GROQ_API_KEY in settings or GROQ_API_KEY environment variable.');
      }
    }
    if (isOpenRouterSelected()) {
      if (isOpenRouterAvailable()) {
        ...
        throw new Error('OpenRouter provider selected but no API key configured. Set ULTRABRAIN_OPENROUTER_API_KEY in settings or OPENROUTER_API_KEY environment variable.');
      }
    }
    if (isGeminiSelected()) {
      if (isGeminiAvailable()) {
        ...
        throw new Error('Gemini provider selected but no API key configured. Set ULTRABRAIN_GEMINI_API_KEY in settings or GEMINI_API_KEY environment variable.');
      }
    }
   ...
   */
  private getSelectedProvider(): 'claude' | 'gemini' | 'openrouter' | 'groq' {
  ...
  private startGeneratorWithProvider(
    ...
    const agent = provider === 'groq' ? this.groqAgent : (provider === 'openrouter' ? this.openRouterAgent : (provider === 'gemini' ? this.geminiAgent : this.sdkAgent));
    const agentName = provider === 'groq' ? 'Groq' : (provider === 'openrouter' ? 'OpenRouter' : (provider === 'gemini' ? 'Gemini' : 'Claude SDK'));
  ...
  private handleSessionInit = this.wrapHandler((req: Request, res: Response): void => {
    ...
    // Get the latest user_prompt for this session to sync to LanceDB
    ...
    if (latestPrompt) {
      ...
      // Sync user prompt to LanceDB
      const lanceStart = Date.now();
      const promptText = latestPrompt.prompt_text;
      this.dbManager.getLanceSync().syncUserPrompt(
        latestPrompt.id,
        ...
        const lanceDuration = Date.now() - lanceStart;
        const truncatedPrompt = promptText.length > 60
          ? promptText.substring(0, 60) + '...'
          : promptText;
        logger.debug('LANCE', 'User prompt synced', {
          promptId: latestPrompt.id,
          duration: `${lanceDuration}ms`,
          prompt: truncatedPrompt
        });
      }).catch((error) => {
        logger.error('LANCE', 'User prompt sync failed, continuing without vector search', {
          promptId: latestPrompt.id,
  ...
  private handleObservationsByClaudeId = this.wrapHandler((req: Request, res: Response): void => {
    ...
    const skipTools = new Set(settings.ULTRABRAIN_SKIP_TOOLS.split(',').map(t => t.trim()).filter(Boolean));
