...
export class OpenRouterAgent {
  ...
  private truncateHistory(history: ConversationMessage[]): ConversationMessage[] {
    ...
    const MAX_CONTEXT_MESSAGES = parseInt(settings.ULTRABRAIN_OPENROUTER_MAX_CONTEXT_MESSAGES) || DEFAULT_MAX_CONTEXT_MESSAGES;
    const MAX_ESTIMATED_TOKENS = parseInt(settings.ULTRABRAIN_OPENROUTER_MAX_TOKENS) || DEFAULT_MAX_ESTIMATED_TOKENS;
  ...
  private getOpenRouterConfig(): { apiKey: string; model: string; siteUrl?: string; appName?: string } {
    ...
    const model = settings.ULTRABRAIN_OPENROUTER_MODEL || 'xiaomi/mimo-v2-flash:free';
