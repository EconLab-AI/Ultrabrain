...
/**
 * UltraBrainDatabase - New entry point for the sqlite module
 *
 * Replaces SessionStore as the database coordinator.
 * Sets up bun:sqlite with optimized settings and runs all migrations.
 *
 * Usage:
 *   const db = new UltraBrainDatabase();  // uses default DB_PATH
 *   const db = new UltraBrainDatabase('/path/to/db.sqlite');
 *   const db = new UltraBrainDatabase(':memory:');  // for tests
 */
export class UltraBrainDatabase {
  public db: Database;
 ...
 * @deprecated Use UltraBrainDatabase instead for new code
 */
