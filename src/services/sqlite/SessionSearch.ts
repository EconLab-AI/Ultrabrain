 ...
 * Vector search is handled by LanceDB - this class only supports filtering without query text
 */
export class SessionSearch {
   ...
   * Vector search (LanceDB) is now the primary search mechanism.
   *
   * Retention Rationale:
   ...
   * Vector search is handled by LanceDB - this only supports filtering without query text.
   */
  searchObservations(query: string | undefined, options: SearchOptions = {}): ObservationSearchResult[] {
    ...
    // This enables date filtering which LanceDB cannot do (requires direct SQLite access)
    if (!query) {
    ...
    // Vector search with query text should be handled by LanceDB
    // This method only supports filter-only queries (query=undefined)
    logger.warn('DB', 'Text search not supported - use LanceDB for vector search');
    return [];
  }
