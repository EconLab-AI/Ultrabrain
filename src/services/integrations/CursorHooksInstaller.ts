/**
 * CursorHooksInstaller - Cursor IDE integration for ultrabrain
 *
...
export function configureCursorMcp(target: CursorInstallTarget): number {
  ...
  try {
    ...
    // Add ultrabrain MCP server
    config.mcpServers['ultrabrain'] = {
      command: 'node',
      args: [mcpServerPath]
    };
...
export async function installCursorHooks(_sourceDir: string, target: CursorInstallTarget): Promise<number> {
  console.log(`\nInstalling UltraBrain Cursor hooks (${target} level)...\n`);
...
Next steps:
  1. Start ultrabrain worker: ultrabrain start
  2. Restart Cursor to load the hooks
  3. Check Cursor Settings → Hooks tab to verify

Context Injection:
  Context from past sessions is stored in .cursor/rules/ultrabrain-context.mdc
...
async function setupProjectContext(targetDir: string, workspaceRoot: string): Promise<void> {
  ...
  if (!contextGenerated) {
    ...
    const rulesFile = path.join(rulesDir, 'ultrabrain-context.mdc');
...
Use ultrabrain's MCP search tools for manual memory queries.
`;
...
export function uninstallCursorHooks(target: CursorInstallTarget): number {
  console.log(`\nUninstalling UltraBrain Cursor hooks (${target} level)...\n`);
  ...
  try {
    ...
    if (target === 'project') {
      const contextFile = path.join(targetDir, 'rules', 'ultrabrain-context.mdc');
...
export function checkCursorHooksStatus(): number {
  console.log('\nUltraBrain Cursor Hooks Status\n');
  ...
  for (const loc of locations) {
    ...
    if (existsSync(hooksJson)) {
      ...
      if (loc.name === 'Project') {
        const contextFile = path.join(loc.dir, 'rules', 'ultrabrain-context.mdc');
        if (existsSync(contextFile)) {
  ...
  if (!anyInstalled) {
    console.log('No hooks installed. Run: ultrabrain cursor install\n');
  }
...
export async function handleCursorCommand(subcommand: string, args: string[]): Promise<number> {
  switch (subcommand) {
    ...
    default: {
      console.log(`
UltraBrain Cursor Integration

Usage: ultrabrain cursor <command> [options]
...
Examples:
  ...
  ultrabrain cursor install user         # Install globally for user
  ultrabrain cursor uninstall            # Remove from current project
  ultrabrain cursor status               # Check if hooks are installed

For more info: https://docs.ultrabrain.ai/cursor
      `);
      return 0;
    }
  }
