...
/**
 * Clean up orphaned ultrabrain processes from previous worker sessions
 *
 * Targets mcp-server.cjs, worker-service.cjs, and lance-worker processes
...
export async function cleanupOrphanedProcesses(): Promise<void> {
  ...
  try {
    if (isWindows) {
      ...
      if (!stdout.trim() || stdout.trim() === 'null') {
        logger.debug('SYSTEM', 'No orphaned ultrabrain processes found (Windows)');
        return;
      }
      ...
      if (!stdout.trim()) {
        logger.debug('SYSTEM', 'No orphaned ultrabrain processes found (Unix)');
        return;
      }
  ...
  logger.info('SYSTEM', 'Cleaning up orphaned ultrabrain processes', {
...
export function spawnDaemon(
  ...
  const env = {
    ...process.env,
    ULTRABRAIN_WORKER_PORT: String(port),
    ...extraEnv
  };
