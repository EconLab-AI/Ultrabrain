 ...
 * - CLI: `ultrabrain generate` / `ultrabrain clean`
...
const DB_PATH = path.join(os.homedir(), '.ultrabrain', 'ultrabrain.db');
const SETTINGS_PATH = path.join(os.homedir(), '.ultrabrain', 'settings.json');
...
function formatObservationsForClaudeMd(observations: ObservationRow[], folderPath: string): string {
  ...
  lines.push('');
  lines.push('<!-- This section is auto-generated by ultrabrain. Edit content outside the tags. -->');
  lines.push('');
...
function writeClaudeMdToFolder(folderPath: string, newContent: string): void {
  ...
  const startTag = '<ultrabrain-context>';
  const endTag = '</ultrabrain-context>';
...
export async function generateClaudeMd(dryRun: boolean): Promise<number> {
  try {
    ...
    const observationLimit = parseInt(settings.ULTRABRAIN_CONTEXT_OBSERVATIONS, 10) || 50;
 ...
 *
 * For each file with <ultrabrain-context> tags:
  ...
  try {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            try {
              ...
              if (content.includes('<ultrabrain-context>')) {
    ...
    for (const file of filesToProcess) {
      ...
      try {
        ...
        const stripped = content.replace(/<ultrabrain-context>[\s\S]*?<\/ultrabrain-context>/g, '').trim();
