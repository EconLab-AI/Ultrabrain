...
export function parseObservations(text: string, correlationId?: string): ParsedObservation[] {
  ...
  while ((match = observationRegex.exec(text)) !== null) {
    ...
    // NOTE FROM ULTRABRAIN: ALWAYS save observations - never skip. 10/24/2025
...
export function parseSummary(text: string, sessionId?: number): ParsedSummary | null {
  ...
  // NOTE FROM ULTRABRAIN: 100% of the time we must SAVE the summary, even if fields are missing. 10/24/2025 
