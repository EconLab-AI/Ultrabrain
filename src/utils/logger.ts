/**
 * Structured Logger for ultrabrain Worker Service
...
export type Component = 'HOOK' | 'WORKER' | 'SDK' | 'PARSER' | 'DB' | 'SYSTEM' | 'HTTP' | 'SESSION' | 'LANCE' | 'FOLDER_INDEX' | 'CLAUDE_MD';
...
const DEFAULT_DATA_DIR = join(homedir(), '.ultrabrain');
...
class Logger {
  ...
  private ensureLogFileInitialized(): void {
    ...
    try {
      ...
      this.logFilePath = join(logsDir, `ultrabrain-${date}.log`);
    } catch (error) {
    ...
    if (this.level === null) {
      try {
        ...
        if (existsSync(settingsPath)) {
          ...
          const envLevel = (settings.ULTRABRAIN_LOG_LEVEL || 'INFO').toUpperCase();
