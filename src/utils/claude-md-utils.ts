...
export async function updateFolderClaudeMdFiles(
  ...
  const limit = parseInt(settings.ULTRABRAIN_CONTEXT_OBSERVATIONS, 10) || 50;

  // Parse exclude paths from settings
  let folderMdExcludePaths: string[] = [];
  try {
    const parsed = JSON.parse(settings.ULTRABRAIN_FOLDER_MD_EXCLUDE || '[]');
    if (Array.isArray(parsed)) {
      folderMdExcludePaths = parsed.filter((p): p is string => typeof p === 'string');
    }
  } catch {
    logger.warn('FOLDER_INDEX', 'Failed to parse ULTRABRAIN_FOLDER_MD_EXCLUDE setting');
  }
  ...
  // See: https://github.com/EconLab-AI/Ultrabrain/issues/859
