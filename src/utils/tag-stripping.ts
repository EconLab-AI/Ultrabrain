 ...
 * 1. <ultrabrain-context> - System-level tag for auto-injected observations
...
function countTags(content: string): number {
  ...
  const contextCount = (content.match(/<ultrabrain-context>/g) || []).length;
...
function stripTagsInternal(content: string): string {
  ...
  return content
    .replace(/<ultrabrain-context>[\s\S]*?<\/ultrabrain-context>/g, '')
