...
export function writeContextFile(workspacePath: string, context: string): void {
  ...
  const rulesFile = join(rulesDir, 'ultrabrain-context.mdc');
...
The following context is from ultrabrain, a persistent memory system that tracks your coding sessions.

${context}

---
*Updated after last session. Use ultrabrain's MCP search tools for more detailed queries.*
`;
...
export function readContextFile(workspacePath: string): string | null {
  const rulesFile = join(workspacePath, '.cursor', 'rules', 'ultrabrain-context.mdc');
...
/**
 * Configure ultrabrain MCP server in Cursor's mcp.json
...
/**
 * Remove ultrabrain MCP server from Cursor's mcp.json
...
export function removeMcpConfig(mcpJsonPath: string): void {
  ...
  try {
    ...
    if (config.mcpServers && config.mcpServers['ultrabrain']) {
      delete config.mcpServers['ultrabrain'];
      writeFileSync(mcpJsonPath, JSON.stringify(config, null, 2));
    }
  } catch (e) {
