/**
 * EnvManager - Centralized environment variable management for ultrabrain
 *
 * Provides isolated credential storage in ~/.ultrabrain/.env
 * This ensures ultrabrain uses its own configured credentials,
...
// Path to ultrabrain's centralized .env file
const DATA_DIR = join(homedir(), '.ultrabrain');
...
// Credential keys that ultrabrain manages
...
function serializeEnvFile(env: Record<string, string>): string {
  const lines: string[] = [
    '# ultrabrain credentials',
    '# This file stores API keys for ultrabrain memory agent',
    '# Edit this file or use ultrabrain settings to configure',
    '',
  ];
...
/**
 * Load credentials from ~/.ultrabrain/.env
 * Returns empty object if file doesn't exist (means use CLI billing)
 */
export function loadUltraBrainEnv(): UltraBrainEnv {
...
/**
 * Save credentials to ~/.ultrabrain/.env
 */
export function saveUltraBrainEnv(env: UltraBrainEnv): void {
  try {
 ...
 * - Adds credentials ONLY from ultrabrain's .env file
...
export function buildIsolatedEnv(includeCredentials: boolean = true): Record<string, string> {
  ...
  // 3. Add credentials from ultrabrain's .env file (NOT from process.env)
  if (includeCredentials) {
    const credentials = loadUltraBrainEnv();

    // Only add ANTHROPIC_API_KEY if explicitly configured in ultrabrain
...
/**
 * Get a specific credential from ultrabrain's .env
 * Returns undefined if not set (which means use default/CLI billing)
 */
export function getCredential(key: keyof UltraBrainEnv): string | undefined {
  const env = loadUltraBrainEnv();
  return env[key];
}

/**
 * Set a specific credential in ultrabrain's .env
 * Pass empty string to remove the credential
 */
export function setCredential(key: keyof UltraBrainEnv, value: string): void {
  const env = loadUltraBrainEnv();
  env[key] = value || undefined;
  saveUltraBrainEnv(env);
}

/**
 * Check if ultrabrain has an Anthropic API key configured
...
export function getAuthMethodDescription(): string {
  if (hasAnthropicApiKey()) {
    return 'API key (from ~/.ultrabrain/.env)';
  }
