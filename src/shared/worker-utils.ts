 ...
 * Uses ULTRABRAIN_WORKER_PORT from settings file or default (37777)
...
export function getWorkerPort(): number {
  ...
  const settingsPath = path.join(SettingsDefaultsManager.get('ULTRABRAIN_DATA_DIR'), 'settings.json');
  const settings = SettingsDefaultsManager.loadFromFile(settingsPath);
  cachedPort = parseInt(settings.ULTRABRAIN_WORKER_PORT, 10);
  return cachedPort;
}

/**
 * Get the worker host address
 * Uses ULTRABRAIN_WORKER_HOST from settings file or default (127.0.0.1)
...
export function getWorkerHost(): string {
  ...
  const settingsPath = path.join(SettingsDefaultsManager.get('ULTRABRAIN_DATA_DIR'), 'settings.json');
  const settings = SettingsDefaultsManager.loadFromFile(settingsPath);
  cachedHost = settings.ULTRABRAIN_WORKER_HOST;
  return cachedHost;
}
 ...
 * See: https://github.com/EconLab-AI/Ultrabrain/issues/811
 */
